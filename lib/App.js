...
class App {
  ...
  async runDocker({
    ...
    const { HOMEY_APP_RUNNER_PATH } = process.env; // e.g. /Users/username/Git/homey-app-runner/src
    ...
    // e.g. /Users/username/Git/node-homey-apps-sdk-v3
    const { HOMEY_APP_RUNNER_SDK_PATH } = process.env;
    ...
    if (!process.env.HOMEY_APP_RUNNER_ID) {
      ...
      if (hours > 12) {
        ...
        await new Promise((resolve, reject) => {
          docker.pull(HOMEY_APP_RUNNER_ID, {}, (dockerPullErr, stream) => {
            if (dockerPullErr) {
              reject(dockerPullErr);
              return;
            }
        ...
        cleanupPromise = Promise.resolve().then(async () => {
          ...
          await Promise.all([
            ...
            Promise.resolve().then(async () => {
              ...
              const container = containers.find(_container => {
                return _container['Labels']['com.athom.session'] === sessionId;
              });
    ...
    const clientIO = await new Promise((resolve, reject) => {
      const _clientIO = SocketIOClient(socketUrl, {
      ...
      _clientIO
        ...
        .on('getFile', ({ filePath }, callback) => {
          Promise.resolve().then(async () => {
            const res = await fetch(`http://localhost:${serverPort}${filePath}`);
            const { status } = res;
            const headers = {
  ...
  async createDriver() {
    ...
    if (App.hasHomeyCompose({ appPath: this.path })) {
      if (driverJson.settings) {
        const driverJsonSettings = driverJson.settings;
      ...
      if (driverJson.flow) {
        const driverJsonFlow = driverJson.flow;
  ...
  static async create({ appPath }) {
    const stat = await statAsync(appPath);
    if (!stat.isDirectory()) {
