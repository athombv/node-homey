...
const LANGUAGES_TRANSLATE = ['nl', 'da', 'de', 'es', 'fr', 'it', 'no', 'sv', 'pl', 'ru', 'ko', 'ar'];
const LANGUAGES = ['en', ...LANGUAGES_TRANSLATE];
  ...
  static async transpileToTypescript({ appPath }) {
    ...
    try {
      ...
      try {
        ...
        throw new Error('Tsconfig validation failed: unable to read configuration from `npx tsc --showConfig`.');
      }
      const actualOutDir = tsconfig.compilerOptions?.outDir;
      const expectedOutDir = './.homeybuild';
      if (actualOutDir !== expectedOutDir) {
        throw new Error(`Expected \`outDir\` to be \`${expectedOutDir}\`, but found \`${actualOutDir || 'undefined'}\``);
      }
...
You can create an API Key on https://platform.openai.com/api-keys.`);
     ...
     */
    function extractTranslationTasks(obj, path = [], languages) {
      const tasks = [];
      ...
      if (typeof obj === 'object' && obj !== null) {
        ...
        for (const key of keys) {
          ...
          tasks.push(
            ...extractTranslationTasks(obj[key], newPath, languages),
          );
        }
      }
     ...
     */
    async function translateOne(lang, text, appName) {
      const ruleSet = [
        'Context:',
    ...
    /**
     * Reorders the translations in the JSON object at the specified path according to LANGUAGES.
     * Preserves the order of existing keys if currentKeys is provided.
     */
    function applyTranslationOrder(obj, path, currentKeys = []) {
      ...
      const sortedKeys = [...allKeys].sort((a, b) => {
        ...
        // If both were already there, preserve their relative order
        if (isExistingA && isExistingB) {
          return currentKeys.indexOf(a) - currentKeys.indexOf(b);
        }

        // If one was already there and the other is new
        if (isExistingA || isExistingB) {
          const existingKey = isExistingA ? a : b;
          const newKey = isExistingA ? b : a;

          const indexExisting = LANGUAGES.indexOf(existingKey);
          const indexNew = LANGUAGES.indexOf(newKey);

          if (indexExisting !== -1 && indexNew !== -1) {
            // Find the correct position for the new key relative to existing keys
            // that are also in LANGUAGES.
            const result = indexA - indexB;

            // If indexA - indexB < 0, then a should come before b.
            // If a is existing and b is new, and indexA < indexB, result is negative, a comes before b. Correct.
            // If a is new and b is existing, and indexA < indexB, result is negative, a comes before b. Correct.
            return result;
          }

          // If one is not in LANGUAGES, put the new one last
          return isExistingA ? -1 : 1;
        }
     ...
     */
    function applyTranslation(obj, path, lang, value) {
      if (!Array.isArray(path)) {
     ...
     */
    async function translateFile(file, languages, appName) {
      const content = await fs.promises.readFile(file, 'utf8');
      const json = JSON.parse(content);

      const tasks = extractTranslationTasks(json, [], languages);

      for (const task of tasks) {
        const currentKeys = Object.keys(task.existing);
        for (const lang of task.missing) {
          const translated = await translateOne(lang, task.source, appName);
          applyTranslation(json, task.path, lang, translated);
        }
        applyTranslationOrder(json, task.path, currentKeys);
      }
    ...
    // Get App name
    const manifest = App.getManifest({ appPath: this.path });
    const appName = getAppName(manifest.name);

    // Start translation
    await Promise.all([...jsonFiles].map(async file => {
      return pqueue.add(async () => {
        try {
          await translateFile(file, languages, appName);
        } catch (err) {
...
App.LANGUAGES_TRANSLATE = LANGUAGES_TRANSLATE;
