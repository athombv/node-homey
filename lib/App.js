...
class App {
  ...
  static async transpileToTypescript({ appPath }) {
    ...
    try {
      await exec('./node_modules/typescript/bin/tsc --outDir build/', { cwd: appPath });
  ...
  static async create({ appPath: cwd }) {
    ...
    const packageJson = {
      ...
      scripts: {
        ...
        lint: answers.eslint ? 'eslint --ignore-path .gitignore .' : undefined,
      },
    };
    ...
    if (answers.typescript) {
      await NpmCommands.installDev(['typescript', '@types/source-map-support'], { appPath });
