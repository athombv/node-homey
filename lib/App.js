...
`);
  ...
  async createWidget() {
    ...
    // const fileExtension = App.usesTypeScript({ appPath: this.path }) ? '.ts' : '.js';
    // const templatePath = path.join(__dirname, '..', 'assets', 'templates', 'app', 'drivers');
    // await copyFileAsync(path.join(templatePath, `driver${fileExtension}`), path.join(widgetPath, `driver${fileExtension}`));
    // await copyFileAsync(path.join(templatePath, `device${fileExtension}`), path.join(widgetPath, `device${fileExtension}`));
