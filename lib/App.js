...
const WidgetServer = require('./WidgetServer');
...
class App {
  ...
  async runDocker({
    ...
    let widgetServer;
    if (widgetHostId) {
      widgetServer = new WidgetServer(widgetHostId, this.path);
      await widgetServer.start();
      const address = widgetServer.getLocalAddress();
      manifest.widgets[widgetHostId].url = address;
    }
