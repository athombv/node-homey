...
class App {
  ...
  async questionDriverWireless({ driverPath, driverJson }) {
    const { isInfrared } = await inquirer.prompt([
      {
        type: 'confirm',
        name: 'isInfrared',
        default: false,
        message: 'Is this an Infrared device?'
      }
    ]);
    ...
    const { isRadio } = await inquirer.prompt([
      {
        type: 'confirm',
        name: 'isRadio',
        default: false,
        message: 'Is this a 433 Mhz/868 Mhz device?'
      }
    ]);
    ...
    const { isZigbee } = await inquirer.prompt([
      {
        type: 'confirm',
        name: 'isZigbee',
        default: false,
    ...
    const { isZwave } = await inquirer.prompt([
      {
        type: 'confirm',
        name: 'isZwave',
        default: false,
