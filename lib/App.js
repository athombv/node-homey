...
class App {
  ...
  async runDocker({ homey, clean, skipBuild, linkModules, network, dockerSocketPath, findLinks, dockerExposedPorts }) {
    // Prepare Docker
  ...
  async startRunnerContainer(
    ...
    const exposedPorts = [];
    if (Array.isArray(dockerExposedPorts)) {
      const portRegex = /^\d+$/;
      const portWithProtocolRegex = /^(\d+)\/(udp|tcp)$/;
      dockerExposedPorts.forEach((port) => {
        let portNumber;
        let portProtocol;
