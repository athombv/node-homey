...
class App {
  ...
  */
  usesTypeScript() {
    return fs.existsSync(path.join(this.path, 'tsconfig.json'));
  }
  ...
  */
  async getAppJson() {
    let hasCompose = App.hasHomeyCompose({ appPath: this.path });
    let appJsonPath;
  ...
  async convertFromTypeScript(appPath) {
    ...
    try {
      ...
      await exec(`cd ${appPath} && npx -p typescript tsc --outDir build`);
  ...
  async install({
    ...
    const usesTypescript = this.usesTypeScript(this.path);
  ...
  static async create({ appPath }) {
    ...
    */
    ...
    await App.addTypes({ appPath }, answers.typescript);
  ...
  static async addTypes({ appPath }, usesTypeScript) {
