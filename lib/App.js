...
class App {
  ...
  static async transpileToTypescript({ appPath }) {
    ...
    const pm = await PackageManager.for(appPath);

    try {
      let tsconfig;
      const showConfigCmd = pm.name === 'pnpm' ? 'pnpm exec' : 'npx';
      try {
        const { stdout } = await pm.exec('tsc --showConfig');
        tsconfig = PackageManager.parseJsonTolerant(stdout, '{');
      } catch (error) {
        const detail = error?.stderr || error?.stdout || error?.message || String(error);
        throw new Error(
          `Tsconfig validation failed: \`${showConfigCmd} tsc --showConfig\` failed:\n${detail}`,
        );
      }
...
`);
  ...
  async preprocess({ copyAppProductionDependencies = true } = {}) {
    ...
    // Announce the chosen package manager right before the first step that uses it.
    const willCopyDeps = copyAppProductionDependencies;
    const willTranspile = App.usesTypeScript({ appPath: this.path });
    if (willCopyDeps || willTranspile) {
      const pm = await PackageManager.for(this.path);
      Log.success(`Using package manager \`${pm.name}\``);
    }
  ...
  async _copyAppProductionDependencies() {
    ...
    if (hasNodeModules === true && hasPackageJSON === false) {
      // Listing prod deps needs a package.json. Without one, copy node_modules wholesale.
      const src = path.join(this.path, 'node_modules');
      const dest = path.join(this._homeyBuildPath, 'node_modules');
      await fse.copy(src, dest);
    } else {
      const pm = await PackageManager.for(this.path);
      await pm.copyProductionDependencies(this._homeyBuildPath).catch((error) => {
        if (error.code === PackageManager.ERROR_CODES.PM_NOT_INSTALLED) {
          throw error;
        }
        Log.error(error.message);
        throw new Error(`This error may be fixed by running \`${pm.name} install\` in your app.`);
      });
    }
  ...
  async installRfLibrary() {
    const pm = await PackageManager.for(this.path);
    await pm.install(['homey-rfdriver']);
  }

  async installZigbeeLibrary() {
    const pm = await PackageManager.for(this.path);
    await pm.install(['homey-zigbeedriver']);
  }

  async installZWaveLibrary() {
    const pm = await PackageManager.for(this.path);
    await pm.install(['homey-zwavedriver']);
  }

  async installOAuth2Library() {
    const pm = await PackageManager.for(this.path);
    await pm.install(['homey-oauth2app']);
  }
  ...
  async changeDriverCapabilities() {
    ...
    const driverJsonPath = path.join(
      this.path,
      'drivers',
  ...
  async createDiscoveryStrategy() {
    ...
    await writeFileAsync(
      path.join(discoveryPath, `${discoveryBase.title}.json`),
      JSON.stringify(discoveryJson, false, 2),
    );
  ...
  static async create({ appPath: cwd, globalAnswers }) {
    ...
    if (installed.length === 0) {
      throw new Error('No package manager found on PATH. Install npm or pnpm.');
    }
    ...
    const localAnswers = await inquirer.prompt([
      ...
      ...(installed.length > 1
        ? [{
          ...
          choices: installed.map((pm) => ({ name: `${pm.name} (${pm.version})`, value: pm })),
        }]
        : []),
      {
        type: 'confirm',
    ...
    // When only one manager is installed there's no prompt; use it silently.
    const chosenPm = localAnswers.packageManager ?? installed[0];
    ...
    const packageJson = {
      ...
      version: '1.0.0',
      main: 'app.js',
      packageManager: `${chosenPm.name}@${chosenPm.version}`,
      scripts: {
        build: undefined,
    ...
    if (answers.eslint) {
      await pm.installDev(['eslint@^7.32.0', 'eslint-config-athom']);
...
${readme}`,
  ...
  static async addTypes({ appPath, useTs }) {
    ...
    if (useTs) {
      ...
      await pm.installDev(['@types/homey@npm:homey-apps-sdk-v3-types']);
    }
  }
  ...
  async addDependencies({ dependencies, dev = false }) {
    const pm = await PackageManager.for(this.path);
    if (dev) {
      await pm.installDev(dependencies);
    } else {
  ...
  async removeDependencies({ dependencies, dev = false }) {
    const pm = await PackageManager.for(this.path);
    await pm.uninstall(dependencies, { dev });
