  ...
  static async _questionFlowArguments() {
    ...
    while (addArgument) {
      ...
      if (['text', 'autocomplete', 'number', 'date', 'time', 'device'].includes(flowCardArgument.type)){
        ...
        flowCardArgument.placeholder = argumentPlaceholder;
      }

      if(flowCardArgument.type == 'number' || flowCardArgument.type == 'range'){
        const {min, max, step} = await inquirer.prompt([
          {
            type: 'input',
            name: 'min',
        ...
        flowCardArgument.min = min;
        flowCardArgument.max = max;
        flowCardArgument.step = step;
      }
      ...
      if (flowCardArgument.type == "dropdown"){
        ...
        while (confirmDropdownValues){
          const {id, label} = await inquirer.prompt([
            {
              type: 'input',
              name: 'id',
            ...
            },
            {
              type: 'input',
              name: 'label',
              message: 'What is the label of the value?',
              validate: input => input.length > 0,
            }
          ])

          dropdownValues.push({id,label});

          const { confirmMoreDropdownValues } = await inquirer.prompt([
            {
              type: 'confirm',
              name: 'confirmDropdownValues',
              message: 'Add another value to you dropdown?',
              default: false
            }
          ]);
      ...
      // flowCard.args = [flowCardArgument];
      flowCard.push(flowCardArgument);
