  ...
  */
  ...
  async convertFromTypeScript(appPath) {
    ...
    try {
      ...
      // await exec(`npx -p typescript tsc --outDir build`, { cwd: appPath });
      await exec(`npm run ts-compile`, { cwd: appPath });
  ...
  async preprocess() {
    ...
    if (App.usesTypeScript({ appPath: this.path })) {
      await this.convertFromTypeScript(this.path);
    }
  ...
  static async create({ appPath }) {
    ...
    */
    ...
    const packageJson = {
      ...
      main: mainFile,
      scripts: {
        "ts-compile": "npx -p typescript tsc --outDir build"
      },
    }
