...
class App {
  ...
  static collectRunnerEnv(inspectPort) {
    return {
      ...
      HOMEY_APP_RUNNER_ID: process.env.HOMEY_APP_RUNNER_ID || 'ghcr.io/athombv/homey-app-runner:latest',
  ...
  async runDocker({
    ...
    // Build the App
    await this.buildForLocalRunner(skipBuild);
  ...
  async startRunnerContainer(
    ...
    if (HOMEY_APP_RUNNER_SDK_PATH !== undefined) {
      containerBinds.push(`${HOMEY_APP_RUNNER_SDK_PATH}:/homey-app-runner/node_modules/@athombv/homey-apps-sdk-v3:ro,z`);
    }
...
${readme}`,
  ...
  async installDependencies() {
    throw new Error('Dependency management is currently only supported for Python apps.');
  }
