  ...
  async createFirmwareUpdate({ driverPath, firmwareFile } = {}) {
    ...
    const { changelog, requireFileVersion } = await inquirer.prompt([
      ...
      },
      {
        ...
        message:
          'Should the app check for the current firmware version on the device before updating?',
        default: false,
      },
    ]);
    ...
    const updatesFilePath = path.join(
      ...
      'drivers',
      selectedDriverId,
      'driver.firmware-updates.compose.json',
    );
    ...
    try {
      ...
      if (err.code === 'ENOENT') {
        ...
        throw new Error(`Error in \`firmware-updates.compose.json.\`:\n${err}`);
      }
    }

    firmwareUpdatesJson.updates.push(updateJson);

    await writeFileAsync(updatesFilePath, JSON.stringify(firmwareUpdatesJson, false, 2));
