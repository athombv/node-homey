...
class App {
  ...
  async runRemote({
    ...
    if (clean) {
      Log(colors.yellow('✓ Purged all Homey App settings'));
    }

    Log(colors.green(`✓ Running \`${this._session.appId}\`, press CTRL+C to quit`));
    Log(colors.grey(` — Profile your app's performance at https://go.athom.com/app-profiling?homey=${homey.id}&app=${this._session.appId}`));
...
`));
  ...
  async install({
    ...
    try {
      ...
      Log(colors.red('✖ Err', err.stack || err.message || err.toString()));
      process.exit();
    }
  }
