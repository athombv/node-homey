...
You can create an API Key on https://platform.openai.com/api-keys.`);
    ...
    if (App.hasHomeyCompose({ appPath: this.path })) {
      jsonFiles.push(path.join(this.path, '.homeycompose', 'app.json'));
    } else {
      jsonFiles.push(path.join(this.path, 'app.json'));
    }
      ...
      await Promise.all(files.map(async file => {
        ...
        if (fileStats.isDirectory()) {
          ...
          if (file === '.homeycompose') {
            jsonFiles.push(path.normalize(`${filePath}/app.json`));
          }
        ...
        if (fileStats.isFile() && filePath.endsWith('.json')) {
          ...
          jsonFiles.push(path.normalize(filePath));
        }
      }));
    };
    ...
    Log(`Found ${jsonFiles.length} JSON files to translate...`);
      ...
      try {
        ...
        const chatResult = await openai.chat.completions.create({
          ...
          messages: [{
            role: 'user',
            content: `       
        Translate the JSON objects, but only those where an "en" property is present. Other two-letter keys are allowed. If there's any other key, skip the object.
        Translate to ${languages.join(', ')}.  //       
        Tone of voice: casual, friendly, helpful.
        Don't overwrite a key if one of these already exists.
        Don't touch any other JSON properties.
        The return amount of keys should be the same as the input, excluding translated keys of course.
        If no translations have been made, return the original JSON file.
        Only output the new JSON file. No text. Don't format it as JSON.
      ...
      try {
        ...
        const chatResult = await openai.chat.completions.create({
          ...
          messages: [{
            role: 'user',
            content: `       
            Translate this text, starting directly after ---, to the language code: ${language}.
            Tone of voice: casual, friendly, helpful.
            Only output the new README file. No text. Don't format it as code.
    
            ---    
            ${readme}`,
          }],
        });
        // console.log(language, translationExists);
        await fs.promises.writeFile(translatedReadmePath, chatResult.choices[0].message.content);
        Log(`✅ Translated README.${language}.txt`);
      } catch (err) {
        Log(`❌ Error translating README.${language}.txt`);
        Log(err);
      }
    }));
  }
