...
class App {
  ...
  static getCreateConfirmationQuestion() {
    return {
      type: 'confirm',
      name: 'confirm',
      message: 'Seems good?',
...
`);
  ...
  async publish({ findLinks, dockerSocketPath } = {}) {
    ...
    try {
      ...
      await this._uploadBuildArchive({
        url,
        method,
        headers,
        archiveStream,
  ...
  async _getPackStream({ appPath = this._homeyBuildPath } = {}) {
    let appSize = 0;
    let numFiles = 0;
  ...
  async migrateToCompose() {
    if (App.hasHomeyCompose({ appPath: this.path }))
      throw new Error('You already have the Compose enabled, no need to run this command.');
    ...
    if (appFlowJson) {
      try {
        if (!(await fse.exists(path.join(this._homeyComposePath, 'flow'))))
          await mkdirAsync(path.join(this._homeyComposePath, 'flow'));
      } catch (err) {
      ...
      FLOW_TYPES.forEach(async (type) => {
        ...
        try {
          if (!(await fse.exists(path.join(this._homeyComposePath, 'flow', type))))
            await mkdirAsync(path.join(this._homeyComposePath, 'flow', type));
        } catch (err) {
    ...
    if (appJson.capabilities) {
      ...
      try {
        if (!(await fse.exists(path.join(this._homeyComposePath, 'capabilities'))))
          await mkdirAsync(path.join(this._homeyComposePath, 'capabilities'));
      } catch (err) {
        Log('Error creating folder', err);
      }
    ...
    if (appJson.discovery) {
      try {
        if (!(await fse.exists(path.join(this._homeyComposePath, 'discovery'))))
          await mkdirAsync(path.join(this._homeyComposePath, 'discovery'));
      } catch (err) {
        Log.error('Error creating folder', err);
      }
  ...
  hasHomeyCompose() {
    return App.hasHomeyCompose({ appPath: this.path });
  }

  async ensureHomeyCompose({ confirmMigration } = {}) {
    if (this.hasHomeyCompose()) {
      return;
    }
    ...
    const shouldMigrate =
      typeof confirmMigration === 'function'
        ...
        : await this._askComposeMigration();
  ...
  getComposeManifest() {
    return App.getComposeManifest({ appPath: this.path });
  }

  getDefaultDriverId(driverName = '') {
    let name = String(driverName ?? '');
  ...
  validateNewDriverId(input) {
    if (String(input ?? '').match(INVALID_CHARACTERS)) {
      throw new Error(
        'Invalid characters: only use letters, numbers, minus (-) and underscore (_)',
      );
    }
  ...
  validateDiscoveryStrategyId(input) {
    const discoveryPath = path.join(this.path, '.homeycompose', 'discovery');

    if (fs.existsSync(path.join(discoveryPath, `${input}.json`))) {
  ...
  parseDiscoveryMacAddresses(value = '') {
    ...
    if (entries.length === 0) {
      throw new Error('At least one MAC address is required');
    }

    entries.forEach((entry) => {
      const isFullMacAddress =
        entry.length === 17 && /^([0-9A-Fa-f]{2}[:-]){5}([0-9A-Fa-f]{2})$/.test(entry);
      const isMacPrefix =
        entry.length === 8 && /^([0-9A-Fa-f]{2}[:-]){2}([0-9A-Fa-f]{2})$/.test(entry);

      if (!isFullMacAddress && !isMacPrefix) {
        throw new Error('Each MAC entry must be a full MAC address or the first three bytes');
      }
    });

    return entries.map((entry) => {
      return Util.parseMacToDecArray(entry);
    });
  }

  buildDiscoveryStrategyJson(strategy = {}) {
    switch (strategy.type) {
      case 'mdns-sd': {
        return {
          ...
          id: this.normalizeTemplateToken(strategy.identifier),
        };
      }
      ...
      case 'mac': {
        const manufacturer = Array.isArray(strategy.macAddresses)
          ? strategy.macAddresses
          : this.parseDiscoveryMacAddresses(strategy.macAddresses);
      ...
      default: {
        throw new Error(`Unsupported discovery strategy type: ${strategy.type}`);
      }
    }
  }

  async createDiscoveryStrategyFromConfig(strategy = {}) {
    const discoveryPath = path.join(this.path, '.homeycompose', 'discovery');
    const discoveryJson = this.buildDiscoveryStrategyJson(strategy);

    if (!(await fse.exists(discoveryPath))) {
      await mkdirAsync(discoveryPath);
    }

    const discoveryJsonPath = path.join(discoveryPath, `${strategy.title}.json`);
    await writeFileAsync(discoveryJsonPath, JSON.stringify(discoveryJson, false, 2));
  ...
  async resolveDriverWirelessConfig(driverConfig = {}) {
    switch (driverConfig.wirelessType) {
      case 'infrared': {
        if (driverConfig.shouldInstallRFDriver) {
          await this.installRfLibrary();
        }
      ...
      case 'zigbee': {
        if (driverConfig.shouldInstallZigbeeDriver) {
          await this.installZigbeeLibrary();
        }
      ...
      case 'zwave': {
        if (driverConfig.shouldInstallZwaveDriver) {
          await this.installZWaveLibrary();
        }

        const { settings, zwave } = ZWave.buildDriverAutocompleteResult(driverConfig);
        ZWave.logAutocompleteDriverHint();
      ...
      case 'oauth2': {
        if (driverConfig.shouldInstallOAuth2App) {
          await this.installOAuth2Library();
        }
      ...
      case 'lan': {
        if (driverConfig.createDiscovery && driverConfig.discoveryStrategy) {
          const { discoveryPath } = await this.createDiscoveryStrategyFromConfig(
            driverConfig.discoveryStrategy,
          );
      ...
      case 'matter': {
        const matter = {
          productId: Number(driverConfig.productId),
          vendorId: Number(driverConfig.vendorId),
        };

        if (driverConfig.isBridgedDevice) {
          matter.deviceProductName = driverConfig.deviceProductName;
          matter.deviceVendorId = Number(driverConfig.deviceVendorId);
        }
  ...
  logDriverCreateHelp(connectivityType) {
  ...
  async createDriverFromConfig(driverConfig = {}) {
    await this.ensureHomeyCompose({
      confirmMigration: async () => {
        return Boolean(driverConfig.shouldMigrateCompose);
      },
    });

    const manifest = this.getComposeManifest();
    const driverWireless = await this.resolveDriverWirelessConfig(driverConfig);
    const driverPath = path.join(this.path, 'drivers', driverConfig.driverId);
    await fse.ensureDir(driverPath);

    const driverJson = {
      name: { en: driverConfig.driverName },
      class: driverConfig.driverClass,
      capabilities: driverConfig.driverCapabilities,
    ...
    await writeFileAsync(
      ...
      JSON.stringify(driverJson, false, 2),
    );

    if (driverWireless.settings) {
      await writeFileAsync(
        path.join(driverPath, 'driver.settings.compose.json'),
        JSON.stringify(driverWireless.settings, false, 2),
      );
    }
    ...
    if (!driverWireless.matter) {
      await this.copyDriverAndDeviceTemplate(templatePath, driverPath);
    }
    ...
    this.logDriverCreateHelp(driverJson.connectivity[0]);
  ...
  getDriverComposeJsonPath(driverId) {
    return path.join(this.path, 'drivers', driverId, 'driver.compose.json');
  }

  async getDriverComposeJson(driverId) {
    const driverJsonPath = this.getDriverComposeJsonPath(driverId);
    let driverJson;
    ...
    try {
      ...
      throw new Error(`Error in \`driver.compose.json\`:\n${err}`);
    }

    return driverJson;
  }

  async setDriverCapabilities(driverId, capabilities = []) {
    const driverJsonPath = this.getDriverComposeJsonPath(driverId);
    const driverJson = await this.getDriverComposeJson(driverId);
    driverJson.capabilities = capabilities;
  ...
  async createDriver() {
    ...
    const { driverId, driverClass, driverCapabilities } = await inquirer.prompt([
      this.getDriverIdPrompt(driverName),
      {
        type: 'list',
        name: 'driverClass',
    ...
    await this.createDriverFromConfig({
      ...driverWireless,
      driverCapabilities,
      driverClass,
      driverId,
  ...
  getDriverIdPrompt(driverName) {
    return {
      ...
      default: () => {
        return this.getDefaultDriverId(driverName);
      },
      validate: (input) => {
        return this.validateNewDriverId(input);
      },
    };
  }
  ...
  async questionDriverWireless() {
    ...
    switch (wirelessType) {
      ...
      case 'zwave': {
        ...
        const zwaveAnswers = await inquirer.prompt([
          {
            type: 'input',
            name: 'manufacturerId',
          ...
          },
          {
            ...
            message:
              'What is the Z-Wave Alliance Product Documentation URL? Leave empty if unknown.',
          },
          {
            type: 'input',
      ...
      case 'lan': {
        ...
        const discoveryStrategy = createDiscovery
          ? await this.promptDiscoveryStrategyConfig()
          : undefined;
      ...
      case 'matter': {
        ...
        if (isBridgedDevice) {
          ...
          driver.deviceVendorId = deviceVendorId;
          driver.deviceProductName = deviceProductName;
        }
  ...
  async changeDriverCapabilities() {
    ...
    const driverJson = await this.getDriverComposeJson(selectedDriverAnswer.driverId);
    ...
    await this.setDriverCapabilities(
      selectedDriverAnswer.driverId,
      capabilitesAnswers.capabilities,
    );
  ...
  async createFlowJson() {
    ...
    const { flowId } = await inquirer.prompt([
      {
        ...
        validate: (input) => {
          ...
          if (
            fs.existsSync(path.join(this.path, '.homeycompose', 'flow', flowType, `${input}.json`))
          ) {
    ...
    const flowTokens = flowType === 'triggers' ? await App._questionFlowTokens() : [];
  ...
  static async _questionFlowArguments() {
    ...
    while (addArgument) {
      ...
      if (
        ['text', 'autocomplete', 'number', 'date', 'time', 'device'].includes(flowCardArgument.type)
      ) {
  ...
  async createFlow() {
    ...
    await writeFileAsync(
      path.join(flowPath, `${flowJson.id}.json`),
      JSON.stringify(flowJson.manifest, false, 2),
    );
  ...
  async createWidget() {
    ...
    const { widgetName } = await inquirer.prompt([
      {
        type: 'input',
        name: 'widgetName',
        message: "What is your Widget's Name?",
        validate: (input) => input.length > 0,
      },
    ]);

    const { widgetId } = await inquirer.prompt([
      {
        type: 'input',
        name: 'widgetId',
        message: "What is your Widget's ID?",
        default: () => {
          let name = widgetName;
          name = name.toLowerCase();
          name = name.replace(/ /g, '-');
    ...
    await this.copyWidgetTemplate(widgetPath, widgetName);
  ...
  async copyWidgetTemplate(widgetPath, widgetName) {
    ...
    await writeFileAsync(
      path.join(widgetPath, 'widget.compose.json'),
    ...
    await copyFileAsync(
      ...
      path.join(widgetPath, 'public/index.html'),
    );
    await copyFileAsync(
    ...
    await copyFileAsync(
      path.join(templatePath, 'preview-light.png'),
      path.join(widgetPath, 'preview-light.png'),
    );
  }

  async createDiscoveryStrategy() {
    await this.ensureHomeyCompose();
    const discoveryStrategy = await this.promptDiscoveryStrategyConfig();
    ...
    const { discoveryPath } = await this.createDiscoveryStrategyFromConfig(discoveryStrategy);

    Log.success(`Discovery strategy created in \`${discoveryPath}\``);
  }

  async promptDiscoveryStrategyConfig() {
    const discoveryBase = await inquirer.prompt([
      {
        ...
        validate: async (input) => {
          return this.validateDiscoveryStrategyId(input);
        },
      },
      {
        type: 'list',
      ...
      answers = await inquirer.prompt([
        {
          ...
          validate: async (input) => {
            if (
              input.length === 17 &&
              input.search(/^([0-9A-Fa-f]{2}[:-]){5}([0-9A-Fa-f]{2})$/) === 0
            )
              return true;
            if (
              input.length === 8 &&
              input.search(/^([0-9A-Fa-f]{2}[:-]){2}([0-9A-Fa-f]{2})$/) === 0
            )
              return true;
      ...
      case 'mdns-sd':
        answers = await inquirer.prompt([
          ...
          },
          {
            ...
            name: 'id',
            message:
              "What is the identifier to identify the device? For example, 'name' or 'txt.id'",
            validate: (input) => {
              return input.length > 0;
            },
          },
      ...
      case 'ssdp':
        answers = await inquirer.prompt([
          ...
          },
          {
            ...
            name: 'id',
            message:
              "What is the identifier to identify the device? For example, 'name' or 'headers.usn'",
            validate: (input) => {
              if (input.match(INVALID_CHARACTERS)) {
                throw new Error(
                  'Invalid characters: only use letters, numbers, minus (-) and underscore (_)',
  ...
  static async create({ appPath: cwd, globalAnswers, localAnswers }) {
    const stat = await statAsync(cwd);
    if (!stat.isDirectory()) {
      throw new Error('Invalid path, must be a directory');
    }

    const resolvedLocalAnswers =
      localAnswers ??
      (await inquirer.prompt([...this.getCreateQuestions(), this.getCreateConfirmationQuestion()]));

    // Combine global and local answers
    const answers = {
      ...globalAnswers,
      ...resolvedLocalAnswers,
    };

    // Add already known answer (inserting it here will make it more readable and understandable when using)
    answers.typescript = answers['programming-language'] === 'typescript';
    ...
    // make dirs
    const dirs = ['locales', 'drivers', 'assets', path.join('assets', 'images')];
    ...
    if (answers.eslint) {
      ...
      await writeFileAsync(
        ...
        JSON.stringify(eslintConfig, false, 2),
      );
    }
    ...
    await writeFileAsync(
      path.join(appPath, '.homeycompose', 'app.json'),
      JSON.stringify(appJson, false, 2),
    );
    ...
    await writeFileAsync(
      ...
      JSON.stringify(generatedAppManifestWarning, false, 2),
    );
    ...
    const files = [path.join('assets', 'icon.svg')];
...
/.homeybuild/`;
    ...
    await writeFileAsync(
      path.join(appPath, '.homeychangelog.json'),
      JSON.stringify(
        {
          [appJson.version]: {
            en: 'First version!',
  ...
  skipFolderForOpenAI(file) {
    return file === 'locales' || file === 'node_modules' || file === '.homeybuild';
  }
        ...
        await Promise.all(
          files.map(async (file) => {
            ...
            if (fileStats.isDirectory()) {
              if (this.skipFolderForOpenAI(file)) {
                return;
              }
...
${content}`,
            ...
            await fs.promises.writeFile(
              file,
              JSON.stringify(JSON.parse(chatResult.choices[0].message.content), false, 2),
            );
    ...
    if (enLocalePathExists) {
      ...
      await Promise.all(
        Object.values(languages).map(async (language) => {
          return pqueue.add(async () => {
            const translatedLocalePath = path.join(this.path, 'locales', `${language}.json`);

            try {
              const translationExists = !!(await fs.promises
                .stat(translatedLocalePath)
                .catch(() => null));
...
${readme}`,
  ...
  static async addTypes({ appPath, useTs }) {
    if (useTs) {
      await NpmCommands.installDev(
        ['@types/homey@npm:homey-apps-sdk-v3-types', '@types/node', '@tsconfig/node16'],
        { appPath },
      );
      ...
      await writeFileAsync(
        ...
        JSON.stringify(tsConfigJson, false, 2),
      );
    } else {
      await NpmCommands.installDev(['@types/homey@npm:homey-apps-sdk-v3-types'], { appPath });
    }
  }
  ...
  static async addGitHubWorkflows({ appPath }) {
    ...
    for (const file of workflows) {
      await copyFileAsync(path.join(workflowsPath, file), path.join(targetPath, file));
    }
  ...
  async addDependencies({ dependencies, dev = false }) {
    const command = dev ? 'npm install -D' : 'npm install';
    await exec(`${command} ${dependencies.join(' ')}`);
  ...
  async removeDependencies({ dependencies, dev = false }) {
    const command = dev ? 'npm uninstall -D' : 'npm uninstall';
    await exec(`${command} ${dependencies.join(' ')}`);
