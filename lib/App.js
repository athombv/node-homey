...
class App {
  ...
  async createDriver() {
    ...
    const wirelessType = await this.questionDriverWireless({ driverPath, driverJson });
    ...
    switch (wirelessType) {
      case 'infrared': {
        Log(`\n\tLearn more about Infrared app developement at: ${colors.underline('https://apps.developer.homey.app/wireless/infrared')}\n`);
        break;
      }

      case 'radio': {
        Log(`\n\tLearn more about 433 Mhz/868 Mhz app developement at: ${colors.underline('https://apps.developer.homey.app/wireless/rf-433mhz-868mhz')}\n`);
        break;
      }

      case 'zigbee': {
        Log(`\n\tLearn more about Zigbee app developement at: ${colors.underline('https://apps.developer.homey.app/wireless/zigbee')}\n`);
        break;
      }

      case 'zwave': {
        Log(`\n\tLearn more about Z-Wave app developement at: ${colors.underline('https://apps.developer.homey.app/wireless/z-wave')}\n`);
        break;
      }

      case 'lan': {
        Log(`\n\tLearn more about LAN app developement at: ${colors.underline('https://apps.developer.homey.app/wireless/wi-fi')}\n`);
        break;
      }
    }
  }
      ...
      if (['number','range'].includes(flowCardArgument.type)) {
        const { argumentMin, argumentMax, argumentStep } = await inquirer.prompt([
          ...
          },
          {
            type: 'input',
            name: 'argumentStep',
            message: 'What is the step value?',
            default: 1,
            validate: input => !Number.isNaN(Number(input))
          },
        ]);
        ...
        let { confirmDropdownValues } = await inquirer.prompt([
          {
            ...
            message: 'Do you want to add values to your dropdown?',
            default: false
          }
        ]);
          ...
          const { confirmMoreDropdownValues } = await inquirer.prompt([
            {
              ...
              message: 'Add another value to your dropdown?',
              default: false
            }
          ]);
