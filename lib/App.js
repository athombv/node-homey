...
class App {
  ...
  async run({
    ...
    const homey = requestedHomey ?? (await AthomApi.getActiveHomey());

    this._activeHomey = homey;
...
`);
  ...
  async publish({ findLinks, dockerSocketPath, athomApi = AthomApi } = {}) {
    const undos = {
      version: null,
    };
  ...
  async _onCtrlC() {
    ...
    try {
      const homey = this._activeHomey ?? (await AthomApi.getActiveHomey());
