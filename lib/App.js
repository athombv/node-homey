...
class App {
  ...
  async runDocker({
    ...
    if (homey.platform === 'local') {
      ...
      fse.watch(userdataDir, (_, filename) => {
        if (userdataDirWarned === false) {
          ...
          Log(colors.yellow('⚠ Warning: The /userdata folder is not synced with Homey Pro while developing.\nAfter running the app from the Homey App Store, your /userdata may be out of sync.'));
        }

        Log(colors.gray(`Modified: ${path.join(userdataDir, filename)}`));
      });
    }
    ...
    const createOpts = {
      ...
      ExposedPorts: {
        '9229/tcp': {},
        ...(Array.isArray(manifest.ports)
          ? manifest.ports.reduce((obj, port) => ({
            ...obj,
            [`${port}`]: {},
          }), {})
          : undefined
    ...
    if (Array.isArray(manifest.ports) && manifest.ports.length) {
      Log(colors.green('✓ Forwarding ports...'));
      manifest.ports.forEach(port => {
        Log(colors.grey(` — Forwarded port ${port} to the local Docker container.`));
      });
    }
...
`));
  ...
  async install({
    ...
    const archiveStream = await this._getPackStream({
      appPath: skipBuild
        ? this.path
        : path.join(this.path, '.homeybuild'),
    });
  ...
  async preprocess() {
    ...
    await fse.remove(path.join(this.path, '.homeybuild')).catch(async err => {
      // It helps to wait a bit when ENOTEMPTY is thrown.
      if (err.code === 'ENOTEMPTY') {
        await new Promise(resolve => setTimeout(resolve, 2000));
        return fse.remove(path.join(this.path, '.homeybuild'));
      }
  ...
  async _getPackStream({
    appPath = path.join(this.path, '.homeybuild'),
  } = {}) {
    let appSize = 0;
    let numFiles = 0;
    ...
    Log(colors.grey(` — App archive size: ${filesize(appSize)}, ${numFiles} files`));
