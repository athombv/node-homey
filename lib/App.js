...
const { Log, Settings, Util, AthomApi } = require('..');
...
class App {
  ...
  static usesTypeScript({ appPath }) {
    return fs.existsSync(path.join(appPath, 'tsconfig.json'));
  }
  ...
  async createDriver() {
    ...
    await this.questionDriverWireless({ driverPath, driverJson });
    ...
    if( App.hasHomeyCompose({ appPath: this.path }) ) {
      ...
      let appJson = await readFileAsync( appJsonPath );
        ...
        appJson.drivers.push({ id: driverId, ...driverJson });
  ...
  async questionDriverWireless({ driverPath, driverJson }) {
    const { wirelessType } = await inquirer.prompt([
      {
        ...
        choices: [
          ...
          { name: '433 Mhz/868 Mhz', value: 'radio' },
    ...
    switch (wirelessType) {
      ...
      case 'lan': {
        ...
        if (createDiscovery) {
          if (App.hasHomeyCompose({ appPath: this.path }) === false) {
            Log(colors.yellow('Creating a Discovery strategy using the Homey CLI requires Homey compose, run `homey app compose` to migrate!'));
            return;
          }
  ...
  static async _questionFlowArguments() {
    ...
    while (addArgument) {
      ...
      if (argumentType != "dropdown" && argumentType != "color" && argumentType != "droptoken" && argumentType != "range"){
          ...
          const { argumentMin, argumentMax, argumentStep } = await inquirer.prompt([
            {
              type: 'input',
              name: 'argumentMin',
              message: 'Enter the minimum value for the argument',
              default: 0,
            ...
            },
            {
              type: 'input',
              name: 'argumentMax',
              message: 'Enter the maximum value for the argument',
              default: 100,
              validate: input => !Number.isNaN(Number(input))
            },
            {
              type: 'input',
              name: 'argumentStep',
              message: 'Enter the step for the argument',
              default: 1,
              validate: input => !Number.isNaN(Number(input))
            },
          ]);
          // Create a custom object without a placeholder.
        flowArguments.push({
          type: argumentType,
          name: argumentName,
      ...
      if(argumentType == "range"){
        ...
        flowArguments.push({
          ...
          const {dropdownTitle} = await inquirer.prompt([
            {
              type: 'input',
              name: 'dropdownTitle',
              message: 'What is the title of the dropdown menu?',
              validate: input => input.length > 0,
            }
          ]);      

          let { addDropdownValues } = await inquirer.prompt([
            {
              type: 'confirm',
              name: 'addDropdownValues',
              message: 'Do you want to add values to your dropdown menu?',
              default: false
            }
          ]);
          ...
          while (addDropdownValues){
            const {dropdownValueId, dropdownValueLabel} = await inquirer.prompt([
              {
                type: 'input',
                name: 'dropdownValueId',
                message: 'Enter the ID of the value',
                validate: input => input.length > 0,
              },
              {
                type: 'input',
                name: 'dropdownValueLabel',
                message: 'Enter the label of the value',
                validate: input => input.length > 0,
              }
            ])

            const { addAnotherDropdownValue } = await inquirer.prompt([
              {
                type: 'confirm',
                name: 'addAnotherDropdownValue',
                message: 'Add another value to you dropdown menu?',
                default: false
              }
            ]);
      
            dropdownValues.push({dropdownValueId,dropdownValueLabel});
            addDropdownValues = addAnotherDropdownValue;
        }
        // Create a custom object to inject a flag for the placeholder.
        flowArguments.push({
          type: argumentType,
          name: argumentName,
          title: dropdownTitle,
  ...
  static async _questionFlowTokens() {
    ...
    while (addToken) {
      ...
      const { tokenType, tokenId } = await inquirer.prompt([
        {
          type: 'list',
          name: 'tokenType',
  ...
  static async create({ appPath }) {
    ...
    /*
    * If typescript is used, then add the types for node and add a tsconfig.json file 
    */
    ...
    const mainFile = answers.typescript ? 'app.ts' : 'app.js';

    await writeFileAsync( path.join(appPath, 'locales', 'en.json'), JSON.stringify({}, false, 2) );
    await writeFileAsync( path.join(appPath, mainFile), '' );
