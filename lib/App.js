...
You can create an API Key on https://platform.openai.com/api-keys.`);
     ...
     */
    function applyTranslationOrder(obj, path) {
      ...
      // Add translations in preferred order
      LANGUAGES.forEach(l => {
        if (l in existing) {
      ...
      // Append any remaining translations
      Object.keys(existing).forEach(l => {
        if (!(l in ordered)) {
          ordered[l] = existing[l];
        }
      });
    ...
    async function translateFile(file, languages, appName) {
      ...
      for (const task of tasks) {
        ...
        applyTranslationOrder(json, task.path);
      }
