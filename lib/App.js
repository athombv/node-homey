      ...
      const walkSync = async dir => {
        ...
        await Promise.all(files.map(async file => {
          ...
          if (fileStats.isFile() && filePath.endsWith('.json')) {
            ...
            // Special handling for driver files
            if (dir.includes('drivers')) {
              // Only include compose files from drivers
              if (file === 'driver.compose.json' || file === 'driver.flow.compose.json' || file === 'driver.settings.compose.json') {
        ...
        try {
          ...
          const chatResult = await openai.chat.completions.create({
            ...
            messages: [{
              role: 'user',
              content: `
This JSON structure may contain JSON Objects with ${languages.join(', ')} as keys. We call them translation objects. 
If on the same level there are other keys then ${languages.join(', ')}, then the object is not a translation object. 
Your task is to translate these translation objects into the following languages: ${languages.join(', ')}.

Strict rules:
1. If a nested translation object does NOT contain an "en" property:
   - Leave it exactly as it is.
   - Do not add or translate any language keys
2. If a nested translation object contains other properties then ${languages.join(', ')}, then skip the object.
3. Only translate translation objects that have an "en" property.
   - If a translation objects has an "en" key, use its value as the source text for translation.
   - If a translation objects does NOT have an "en" key, **do not translate it, do not add new keys, and leave it exactly as it is**.
4. Do not overwrite existing translations from ${languages.join(', ')} keys. For example, if an object already has "nl": "Hallo wereld!".
   - keep it unchanged.
   - don't attempt to fix wrong translations. 
5. Use "en" (English) as the base translation.
6. Never translate IDs between [[ and ]].
7. Don't translate Brand names. For example, "MyBrand" should stay as "MyBrand".
8. Never insert characters from another language or alphabet.
9. Do not “correct” or “guess” translations; only translate using the provided English text as source.
10. Tone of voice: casual, friendly, helpful.
11. Always return the original file exactly as submitted. The only changes may be newly added languages.
12. Only output the new JSON file. No text. Don't format it as JSON. Don't output === START FILE ===.
