      ...
      case 'mdns-sd':
        answers = await inquirer.prompt([
          ...
          },
          {
            ...
            name: 'id',
            message:
              "What is the indentifier to indentify the device? For example, 'name' or 'txt.id'",
            validate: (input) => {
              return input.length > 0;
            },
          },
      ...
      case 'ssdp':
        answers = await inquirer.prompt([
          ...
          },
          {
            ...
            name: 'id',
            message:
              "What is the indentifier to indentify the device? For example, 'name' or 'headers.usn'",
            validate: (input) => {
              if (input.match(INVALID_CHARACTERS)) {
                throw new Error(
                  'Invalid characters: only use letters, numbers, minus (-) and underscore (_)',
