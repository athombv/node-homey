...
const SLIM_EXTENSIONS = ['.d.ts', '.d.mts', '.d.cts', '.map'];

function hasSlimExtension(filePath) {
  return SLIM_EXTENSIONS.some((ext) => filePath.endsWith(ext));
}
...
class App {
  ...
  async build({ slim = false } = {}) {
    Log.success('Building app...');
    await this.preprocess({ slim });
...
`);
  ...
  async install({ homey, clean = false, skipBuild = false, debug = false, slim = false } = {}) {
  ...
  async preprocess({ copyAppProductionDependencies = true, slim = false } = {}) {
    ...
    if (copyAppProductionDependencies) {
      await this._copyAppProductionDependencies({ slim });
    }
  ...
  async _copyAppProductionDependencies({ slim = false } = {}) {
    ...
    if (hasNodeModules === true && hasPackageJSON === false) {
      ...
      await fse.copy(src, dest, {
        filter(copySrc) {
          if (slim && hasSlimExtension(copySrc)) return false;
          return true;
        },
      });
      return;
    ...
    for (const filePath of dependencies) {
      ...
      await fse.copy(fullSrc, fullDest, {
        filter(src) {
          ...
          if (subPath.startsWith('node_modules', 1)) return false;
          if (slim && hasSlimExtension(src)) return false;
          return true;
        },
      });
    }
  ...
  async publish({ findLinks, dockerSocketPath, slim = false } = {}) {
    const undos = {
      version: null,
    };
    ...
    try {
      ...
      await this.preprocess({ findLinks, dockerSocketPath, slim });
  ...
  static async create({ appPath: cwd, globalAnswers }) {
    ...
    if (answers.eslint) {
      await NpmCommands.installDev(['eslint@~8.57.1', 'eslint-config-athom@~4.0.2'], { appPath });
...
${readme}`,
  ...
  static async addTypes({ appPath, useTs }) {
    ...
    if (useTs) {
      ...
      tsConfigJson.extends = '@tsconfig/node16/tsconfig.json';
    } else {
