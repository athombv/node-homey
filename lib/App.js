...
3. Skip interview json files. 
4. Only translate translation objects that have an "en" property.
...
5. Do not overwrite existing translations from ${languages.join(', ')} keys. For example, if an object already has "nl": "Hallo wereld!".
   - keep it unchanged.
   - don't attempt to fix wrong translations. 
6. Use "en" (English) as the base translation.
7. Don't overwrite/modify a value of an existing language property. For example "nl": "hallo wereld" should stay as "hallo wereld". 
8. Never translate IDs between [[ and ]].
9. Don't translate Brand names. For example, "MyBrand" should stay as "MyBrand".
10. Never insert characters from another language or alphabet.
11. Do not “correct” or “guess” translations; only translate using the provided English text as source.
12. Tone of voice: casual, friendly, helpful.
13. Always return the original file exactly as submitted. The only changes may be newly added languages.
14. Only output the new JSON file. No text. Don't format it as JSON. Don't output === START FILE ===.
