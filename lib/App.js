  ...
  static async create({ appPath }) {
    ...
    const answers = await inquirer.prompt([
      ...
      },
      {
        ...
        name: 'eslint',
        message: 'Use ESLint?',
        when: answers => answers.typescript === false,
        default: true,
      },
      {
        type: 'confirm',
    ...
    */
    ...
    if (answers.eslint) {
      await writeFileAsync(path.join(appPath, '.eslintrc.json'), JSON.stringify({  ['extends']: 'athom' }, false, 2));
    }
