      ...
      if (typeof obj === 'object' && obj !== null) {
        ...
        if (isTranslationObject) {
          if ('en' in obj) {
            ...
            Log(`Translations required for ${path}`, missing);
            if (missing.length > 0) {
              tasks.push({
                path,
                source: obj.en,
    ...
    async function translateOne(lang, text) {
      ...
      const result = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: `Translate the following English text into [${lang}]. 
Always preserve [[PLACEHOLDERS]]. 
Do translate enums within brackets, for example: {{yes/no}}.
No brand name translation. 
Do not add extra braces around the non-translated text.
...
Only output the translation, nothing else.`,
     ...
     */
    async function translateFile(file, languages) {
      ...
      for (const task of tasks) {
        for (const lang of task.missing) {
          const translated = await translateOne(lang, task.source);
    ...
    await Promise.all([...jsonFiles].map(async file => {
      return pqueue.add(async () => {
        try {
          await translateFile(file, languages);
        } catch (err) {
