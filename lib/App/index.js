...
class App {
  ...
  /* 
  * Returns true if typescript is set to true in the app.json 
  */
  async usesTypeScript() {
    const json = await this.getAppJson();
    return json.typescript;
  }

  /* 
  * Returns the json object found in app.json 
  */
  async getAppJson() {
    ...
    const appJson = await readFileAsync(appJsonPath, 'utf8');
    return JSON.parse(appJson);
  }

  async convertFromTypeScript(appPath) {
    Log(colors.green('✓ Typescript detected. Running tsc...'));
  
    try {
      const newPath = path.join(appPath, 'ts-output/');
      await exec(`rm -r ${newPath}`).catch(err => {});
      await exec(`cd ${appPath} && tsc`);
      // await exec(`cp ${appPath}/app.json ${newPath}/app.json`);
      // await exec(`cp -r ${appPath}/locales/ ${newPath}/locales/`);
      // await exec(`cp -r ${appPath}/assets/ ${newPath}/assets/`);

      appPath = newPath;
      Log(colors.green('✓ Typescript compilation succesful'));
      return appPath;
    } catch (err) {
  ...
  async install({
    ...
    const usesTypescript = await this.usesTypeScript(this.path);

    // change the source path if typescript is found
    if (usesTypescript) {
      const newSrcPath = await this.convertFromTypeScript(this.path);
      this.srcPath = path.resolve(newSrcPath);
    }
  ...
  async _getPackStream() {
    return tmp.file().then( async o => {
      ...
      let tarOpts = {
        ...
        // Move every file in ts-output to the main directory of the tar
        map: function(header) {
          if (header.name.startsWith('ts-output/'))
          header.name = header.name.replace('ts-output/', '');
          return header
        }
      };
  ...
  async _getPackFileEntries(prunePaths) {
    ...
    const ignoreRules = ['.*', '*.ts', 'env.json', ...prunePaths];
  ...
  async _askComposeMigration() {
    ...
    const fileExtension = this.usesTypeScript() ? '.ts' : '.js';
  ...
  static async create({ appPath }) {
    ...
    let localizedProperties = {
      ...
      },
      {
				type: 'confirm',
				name: 'typescript',
				message: 'Use TypeScript instead of Javascript (experimental)?'
			},
			{
				type: 'confirm',
				name: 'license',
