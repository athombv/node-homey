...
class App {
  ...
  async checkForTypeScript(appPath) {
    return fs.promises.access(path.join(appPath, 'tsconfig.json'), fs.constants.F_OK)
    .then(() => true)
    .catch(() => false);
  }
  ...
  async convertFromTypeScript(appPath) {
    ...
    try {
      ...
      await exec(`rm -r ${newPath}`);
      await exec(`cd ${appPath} && tsc`);
      await exec(`cp -r ${appPath}/.homeycompose ${newPath}/.homeycompose`);
      await exec(`cp ${appPath}/app.json ${newPath}/app.json`);
      await exec(`cp ${appPath}/README.md ${newPath}/README.md`);
      await exec(`cp ${appPath}/README.txt ${newPath}/README.txt`);
  ...
  async install({
    ...
    const usesTypescript = await this.checkForTypeScript(this.path);
    if (usesTypescript) {
      const newpath = await this.convertFromTypeScript(this.path);
