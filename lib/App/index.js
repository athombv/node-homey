...
class App {
  ...
  async preprocess() {
    ...
    HomeyCompose.build({ appPath: this.path });
  }
    ...
    let answers = await inquirer.prompt(
      {
        type: 'confirm',
        name: 'switch_compose',
        message: 'The Homey compose is not detected. Do you want to use Homey compose? It will split the app.json file into separate files for Drivers, Flow Cards and Discovery Strategies.'
      }
    )
      ...
      description: await App._getTranslatedString({ message: 'What is your app\'s description?', defaultAnswer: 'Adds support for MyBrand devices.'})
		...
		let answers = await inquirer.prompt([
			...
			},
			{
				type: 'confirm',
				name: 'compose',
				message: 'Use Homey compose?' // TODO!!!! don't ask, new users don't know what this is
			},
			{
				type: 'confirm',
				name: 'license',
