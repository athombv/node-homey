...
const APP_CLASS_PATH = 'app.js';
const APP_API_PATH = 'api.js';

const APP_ENTRY_PATTERNS = ['drivers/*/driver.js', 'drivers/*/device.js'];
...
class App {
  ...
  /**
   * Returns the relative paths to all discovered app files.
   */
  async _getPackFileEntries() {
    // TODO: is there a way we can provide nodeFileTrace with relative paths?
    // That would allow us to skip resolving all the paths
    const appFiles = [path.resolve(this.path, APP_JSON_PATH)];

    if (fs.existsSync(APP_CLASS_PATH)) {
      appFiles.push(path.resolve(this.path, APP_CLASS_PATH));
    }

    if (fs.existsSync(APP_API_PATH)) {
      appFiles.push(path.resolve(this.path, APP_API_PATH));
    }

    for (const assetPattern of APP_ASSET_PATTERNS) {
      const foundAssets = glob.sync(assetPattern, { cwd: this.path, absolute: true });
      appFiles.push(...foundAssets);
    }

    for (const entryPattern of APP_ENTRY_PATTERNS) {
      const foundEntries = glob.sync(entryPattern, { cwd: this.path, absolute: true });
      appFiles.push(...foundEntries);
    }

    const { fileList } = await nodeFileTrace(appFiles, { base: this.path });
