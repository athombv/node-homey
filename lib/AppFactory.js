...
function pickAnswersWithConfirmation(source = {}, questions = [], confirmationQuestion) {
  const pickedAnswers = pickAnswers(source, [...questions, confirmationQuestion]);

  if (!Object.prototype.hasOwnProperty.call(pickedAnswers, confirmationQuestion.name)) {
    pickedAnswers[confirmationQuestion.name] = confirmationQuestion.default ?? true;
  }
...
class AppFactory {
  ...
  static async createNewAppInstanceFromAnswers({ appPath: cwd, answers }) {
    ...
    if (answers['programming-language'] === 'python') {
      const confirmationQuestion = AppPython.getCreateConfirmationQuestion();

      await AppPython.create({
        appPath: cwd,
        globalAnswers,
        localAnswers: pickAnswersWithConfirmation(
          answers,
          AppPython.getCreateQuestions(),
    ...
    const confirmationQuestion = App.getCreateConfirmationQuestion();
