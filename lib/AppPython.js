...
class AppPython extends App {
  ...
  async buildForLocalRunner(skipBuild, dockerSocketPath) {
    ...
    await this.preprocess({ copyAppProductionDependencies: false, dockerSocketPath });
  }
  ...
  async startRunnerContainer(
    ...
    if (!process.env.HOMEY_APP_RUNNER_ID_PYTHON) {
      ...
      if (!await DockerHelper.imageExists(HOMEY_APP_RUNNER_ID) || await DockerHelper.imageNeedPull(HOMEY_APP_RUNNER_ID)) {
  ...
  async install({
    ...
    await this.preprocess({ copyAppProductionDependencies: true, findLinks, dockerSocketPath });
  ...
  async preprocess(
    ...
    if (copyAppProductionDependencies) {
      await this._copyAppProductionDependencies(findLinks, dockerSocketPath);
    }
  ...
  async _copyAppProductionDependencies(findLinks = undefined, dockerSocketPath = undefined) {
    const cachePath = path.join(this.path, PYTHON_CACHE_FOLDER, 'remote');

    Log.success('Cross-compiling...');
    await AppPython.uvCommand(
      ['uv', 'sync', '--active', '--no-dev', '-q'],
      this._homeyBuildPath,
      {
        forProduction: true,
        pythonCachePath: cachePath,
        findLinks,
        dockerSocketPath,
      },
    )
      .catch((error) => {
        if (error.endsWith('exec format error')) {
    ...
    const venvCachePath = path.join(cachePath, '.venv');
  ...
  async version(version) {
    if (['patch', 'minor', 'major'].includes(version)) {
      await AppPython.uvCommand(['uv', 'version', '--bump', version], this.path).catch();
    } else {
      await AppPython.uvCommand(['uv', 'version', version], this.path).catch();
    }
  ...
  async _getPackStream({
    ...
    await pipeline(
      tar.pack(appPath, {
        ...
        ignore(name) {
          // Include just the packages and ignore the rest of the venv
          if (name.includes('/.venv/lib64')) return true;
          if (name.includes('/.venv/lib')) return false;
          if (name.includes('/.venv/')) return true;
  ...
  static async create({ appPath: cwd, globalAnswers }) {
    ...
    await AppPython.uvCommand(['uv', 'init', '--bare', '--no-description', '-q', '--python', '==3.13', '.'], appPath);
    await AppPython.uvCommand(['uv', 'version', '--active', '-q', '1.0.0'], appPath);
  ...
  async installDependencies({ findLinks, dockerSocketPath, silent = false }) {
    await AppPython.uvCommand(['uv', 'sync', '--active', '--no-dev', '-q'], this.path, { findLinks, dockerSocketPath });
  ...
  async addDependencies({
    ...
    await AppPython.uvCommand(command, this.path, { findLinks, dockerSocketPath });
  ...
  async listDependencies() {
    const list = await AppPython.uvCommand(['uv', 'tree', '-q'], this.path);
    Log(list);
  }

  static async uvCommand(command, projectPath, {
    findLinks, pythonCachePath, forProduction, dockerSocketPath,
  } = {}) {
    const { HOMEY_APP_BUILDER_ID, HOMEY_APP_IGNORE_PYTHON_CACHE } = AppPython.collectRunnerEnv();
    ...
    if (!process.env.HOMEY_APP_BUILDER_ID_PYTHON) {
      // Check if the image exists, or needs refresh pull
      if (!await DockerHelper.imageExists(HOMEY_APP_BUILDER_ID) || await DockerHelper.imageNeedPull(HOMEY_APP_BUILDER_ID)) {
        await DockerHelper.imagePull(HOMEY_APP_BUILDER_ID);
      }
    }
    ...
    if (!HOMEY_APP_IGNORE_PYTHON_CACHE) {
      const cachePath = pythonCachePath ?? path.join(projectPath, PYTHON_CACHE_FOLDER, 'local');
      const venvPath = path.join(cachePath, '.venv');
      const uvCachePath = path.join(cachePath, 'uv_cache');
      await fse.ensureDir(venvPath);
      await fse.ensureDir(uvCachePath);
    ...
    if (forProduction) {
      createOptions['platform'] = 'arm64';
    }

    const passThrough = new stream.PassThrough({ encoding: 'utf8' });

    let runResolve; let runReject;
