...
class AppPython extends App {
  ...
  static async checkHomeyCompatibility(homey) {
    if (!(homey.__properties.platformVersion === 2 && homey.__properties.apiVersion === 3)) {
      throw new StacklessError('Python apps are currently only supported on the Homey Pro and SHS');
    }
  }
  ...
  async preprocess(
    ...
    const hasDependencies = manifest.pythonDependencies !== undefined && manifest.pythonDependencies.length > 0;
    if (copyAppProductionDependencies && hasDependencies) {
      await this.checkPythonVersion(this.path);
  ...
  async syncPyproject(pyprojectPath) {
    ...
    const pyproject = {
      project: {
        ...
        dependencies: manifest.pythonDependencies ?? [],
      },
    };
  ...
  async addDependencies({
    ...
    // Take the union of dependencies
    manifest.pythonDependencies = Array.from(new Set(manifest.pythonDependencies).union(new Set(dependencies)));

    await writeFileAsync(manifestPath, JSON.stringify(manifest, false, 2));
  ...
  async removeDependencies({
    ...
    // Take the difference of dependencies
    manifest.pythonDependencies = Array.from(new Set(manifest.pythonDependencies).difference(new Set(dependencies)));
  ...
  async listDependencies() {
    ...
    const dependencies = manifest.pythonDependencies ?? [];
    Log(dependencies.join('\n'));
  }

  static getCompilationPlatforms() {
    if (arch === 'x64' || arch === 'arm64') {
      return HomeyLibApp.REQUIRED_PYTHON_PLATFORMS;
    }
    return [undefined, ...HomeyLibApp.REQUIRED_PYTHON_PLATFORMS];
  }
  ...
  static getLocalPlatform() {
    switch (arch) {
      case 'x64':
        return 'amd64';
      default:
        // TODO decide what to do with unsupported architectures
        return 'arm64';
    }
  }
  ...
  async uvCommand(command, {
    ...
    // TODO use fallback for local=undefined
    ...
    const cacheBasePath = path.join(this.path, PYTHON_CACHE_FOLDER);
    ...
    for (const platform of platforms) {
      ...
      try {
        ...
        try {
          const result = await this.executeUvCommand(docker, BUILDER_IMAGES[platform], dockerCommand, createOptions, platform);
          if (onlyForLocal) {
            return result;
          }
        } finally {
  ...
  async executeUvCommand(docker, HOMEY_APP_BUILDER_ID, dockerCommand, createOptions, platform, retry = false) {
    ...
    const result = await runPromise.catch(async (error) => {
    ...
    if (retry) {
      return this.executeUvCommand(docker, HOMEY_APP_BUILDER_ID, dockerCommand, createOptions, platform, true);
    }
  ...
  async checkPythonVersion(projectPath) {
    let manifest;
    try {
