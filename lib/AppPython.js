...
const PythonCommands = require('./PythonCommands');
...
class AppPython {
  ...
  async build({ platform = HOMEY_PLATFORMS.ALL } = {}) {
  ...
  async runRemote({
    ...
    AppPython.monitorCtrlC(this._onCtrlC.bind(this));
  }
  ...
  async runDocker({
    ...
    await this.preprocess({
      platform: undefined, // Local architecture
    });
    ...
    const manifest = AppProject.getManifest({ appPath: this.path });
    ...
    const HOMEY_APP_RUNNER_PATH = process.env.HOMEY_APP_RUNNER_PATH_PYTHON; // e.g. /Users/username/Git/homey-app-runner/src
    const HOMEY_APP_RUNNER_CMD = ['python3', 'main.py'];
    const HOMEY_APP_RUNNER_ID = process.env.HOMEY_APP_RUNNER_ID_PYTHON || 'ghcr.io/athombv/python-homey-app-runner:latest';
    const HOMEY_APP_RUNNER_SDK_PATH = process.env.HOMEY_APP_RUNNER_SDK_PATH_PYTHON; // e.g. /Users/username/Git/node-homey-apps-sdk-v3

    // Download Image (if there is no local override)
    if (!process.env.HOMEY_APP_RUNNER_ID_PYTHON) {
      throw new Error('Python app-runner image is not available yet.\nPlease use the environment variable HOMEY_APP_RUNNER_ID_PYTHON to set the local image name.');
    ...
    const createOpts = {
      name: `homey-app-runner-${sessionId}-${manifest.id}-v${manifest.version}-py`,
      Env: [
        'APP_PATH=/app',
      ...
      // Python had no inspect tools (as far as I know)
      // ExposedPorts: {
      //   [`${inspectPort}/tcp`]: {},
      // },
      Labels: {
      ...
      HostConfig: {
        ...
        Binds: [
          ...
          ...(HOMEY_APP_RUNNER_SDK_PATH
            ? [`${HOMEY_APP_RUNNER_SDK_PATH}:/usr/local/lib/python3.13/site-packages/homey_apps_sdk_v3:ro,z`]
            : []),
    ...
    // await docker.run(HOMEY_APP_RUNNER_ID, ['sleep', 'infinity'], passThrough, createOpts);
  ...
  async install({
    ...
    await this.preprocess({
      platform: HOMEY_PLATFORMS.MANYLINUX_ARM64, // Homey Pro
    });
  ...
  async preprocess({
    ...
    if (AppProject.hasHomeyCompose({ appPath: this.path }) === false) {
      // Note: this checks that we are in a valid homey app folder
      AppProject.getManifest({ appPath: this.path });
    }
    ...
    const manifest = AppProject.getManifest({ appPath: this.path });
    // Cross-compiling all platforms (the app is able to run)
    if (platform === HOMEY_PLATFORMS.ALL) {
      platforms = [
        ...(manifest.platforms || []).includes('local') ? [HOMEY_PLATFORMS.MANYLINUX_AMD64] : [],
    ...
    // And also compile them to .homeybuild/python_packages_<platform>/
    if (copyAppProductionDependencies) {
      const startBuilding = performance.now();
      const activeCompilers = [];
      const allCompilers = [];
      const MAX_CROSSCOMPILERS = 2;
      for (platform of platforms) {
        if (activeCompilers.length >= MAX_CROSSCOMPILERS) {
          // Wait for one of the active compilers to finish, before starting a new one
          // TODO: What if the 'splice' happens before this 'race'? (Maybe it never resolves)
          await Promise.race(activeCompilers);
        }

        const compiler = this._copyAppProductionDependencies({ platform });
        activeCompilers.push(compiler);
        allCompilers.push(compiler);

        // Remove from active after done
        compiler.then(() => {
          activeCompilers.splice(activeCompilers.indexOf(compiler), 1);
        });

        // await this._copyAppProductionDependencies({ platform });
      }

      // Wait for all compilers to finish
      await Promise.all(allCompilers);

      if (platforms.length > 1) {
        Log.success(`All App dependencies built in ${performance.now() - startBuilding} ms`);
      }
    }

    // Create __init__.py for the module
    const pythonAssetsPath = path.join(__dirname, '..', 'assets', 'python');
    const fullSrc = path.join(pythonAssetsPath, '__init__.py');
    const fullDest = path.join(this._homeyBuildPath, '__init__.py');
    ...
    if (await fse.pathExists(gitIgnorePath)) {
      ...
      if (!gitIgnore.includes('.homeyvenv')) {
        Log.success('Automatically added `/.homeyvenv/` to .gitignore');
        await fse.writeFile(gitIgnorePath, `${gitIgnore}\n\n# Added by Homey CLI\n/.homeyvenv/`);
      }
    }
  }
  ...
  async _copyAppProductionDependencies({ platform = undefined } = {}) {
    // Read dependencies from manifest
    const manifest = AppProject.getComposeManifest({ appPath: this.path });
    // const pipRequirements = manifest.pipRequirements || [];
    const pipFreeze = manifest.pipFreeze || [];

    if (pipFreeze.length === 0) {
      Log.success('No dependencies found, skipping compiling dependencies');
      return;
    }

    // Write requirements.txt for cross-compiler to use
    await writeFileAsync(path.join(this._homeyBuildPath, 'requirements.txt'), pipFreeze.join('\r\n'));

    // Check current architecture
    if (platform === undefined) {
      if (os.arch() === 'arm64') {
        platform = HOMEY_PLATFORMS.MANYLINUX_ARM64;
      } else {
        platform = HOMEY_PLATFORMS.MANYLINUX_AMD64;
      }
    }

    await PythonCommands.crossCompile({
      buildPath: this._homeyBuildPath,
      platform,
    });
  }
  ...
  async version(version) {
    ...
    if (AppProject.hasHomeyCompose({ appPath: this.path })) {
      try {
        ...
        manifest = AppProject.getComposeManifest({ appPath: this.path });
    ...
    if (!manifest) {
      manifest = AppProject.getManifest({ appPath: this.path });
  ...
  async commit(changelog) {
    if (await GitCommands.isGitInstalled() && await GitCommands.isGitRepo({ path: this.path })) {
      ...
      const { version } = AppProject.getManifest({ appPath: this.path });
  ...
  async publish() {
    ...
    try {
      ...
      let manifest = AppProject.getManifest({ appPath: this.path });
  ...
  async migrateToCompose() {
    if (AppProject.hasHomeyCompose({ appPath: this.path })) throw new Error('You already have the Compose enabled, no need to run this command.');
    ...
    const appJson = AppProject.getManifest({ appPath: this.path });
    ...
    switch (wirelessType) {
      case 'infrared': {
        ...
        if (shouldInstallRFDriver) {
          ...
          // await PythonCommands.pipInstall(['homey-rfdriver'], { cwd: this.path });
        }
      ...
      case 'zigbee': {
        ...
        if (shouldInstallZigbeeDriver) {
          ...
          // await PythonCommands.pipInstall(['homey-zigbeedriver'], { cwd: this.path });
        }
      ...
      case 'zwave': {
        ...
        if (shouldInstallZwaveDriver) {
          ...
          // await PythonCommands.pipInstall(['homey-zwavedriver'], { cwd: this.path });
        }
      ...
      case 'oauth2': {
        ...
        if (shouldInstallOAuth2App) {
          ...
          // await PythonCommands.pipInstall(['homey-oauth2app'], { cwd: this.path });
        }
  ...
  async createFlowJson() {
    ...
    const flowArguments = await AppPython._questionFlowArguments();

    const flowTokens = flowType === 'triggers'
      ? await AppPython._questionFlowTokens()
      : [];
  ...
  static async create({ appPath: cwd, globalAnswers, pythonExecutable = undefined }) {
    ...
    const appJson = {
      id: answers.id,
      version: '1.0.0',
      compatibility: '>=5.0.0', // TODO: Update to Homey version that will support Python
      sdk: 3,
      runtime: 'python',
    ...
    await PythonCommands.venv.create(appPath, pythonExecutable);

    // Editor specific configs
    if (answers['ide'] === 'vscode') {
      // await writeFileAsync(
      //   path.join(appPath, '.vscode', 'settings.json'),
      //   JSON.stringify({
      //     'python.analysis.extraPaths': [
      //       './.homeybuild/python_packages_manylinux_arm64',
      //     ],
      //   }, false, 2),
      // );
    } else if (answers['ide'] === 'pycharm') {
      // TODO: See if this is possible to automate
      // Looks like you can only do it manually
      // File -> Settings -> Project: <ProjectName> -> Python Interpreter ->
      // Click dropdown "show all" -> Select interpreter -> Click icon "Show Interpreter Paths" -> Select the folder
      // await writeFileAsync(
      //   path.join(appPath, '.idea', 'settings.xml'),
      //   JSON.stringify({
      //     'python.analysis.extraPaths': [
      //       './.homeybuild/python_packages_manylinux_arm64',
      //     ],
      ...
      Log.warning('Your code-editor is unknown. Please manually add the following path to the Python module resolution, to use autocompletions:\n"./.homeypython/python_packages_manylinux_arm64"');
    }

    if (answers['github-workflows']) {
      await AppPython.addGitHubWorkflows({ appPath });
    }
    ...
    // Install stubs
    await AppPython.addTypes({ appPath, executable: pythonExecutable });
      ...
      const walkSync = async dir => {
        ...
        await Promise.all(files.map(async file => {
          ...
          if (fileStats.isDirectory()) {
            ...
            if (file === 'python_packages') return;
            if (file === '.homeybuild') return;
            if (file === '.homeyvenv') return;
...
${readme}`,
  ...
  static async addTypes({ appPath, executable = undefined }) {
    // TODO: Publish to PyPI and adjust installation to correct command
    // Install stub-package
    // await PythonCommands.pipInstall(['homey-apps-sdk-v3-stubs'], {
    // -- Start temporary code --
    if (process.platform === 'linux') {
      await PythonCommands.pipInstall(['/mnt/c/Users/lars/Documents/Afstuderen/Git/python-homey-apps-sdk/stubs/dist/homey_apps_sdk_v3_stubs-0.0.1-py3-none-any.whl'], {
        appPath,
        executable,
      });
    } else {
      await PythonCommands.pipInstall(['C:\\Users\\lars\\Documents\\Afstuderen\\Git\\python-homey-apps-sdk\\stubs\\dist\\homey_apps_sdk_v3_stubs-0.0.1-py3-none-any.whl'], {
        appPath,
        executable,
      });
    }
    ...
    // Add mockup module 'homey', that redirects to the stubs
    await PythonCommands.copyIntoSitePackages(path.join(__dirname, '..', 'assets', 'python', 'homey'), { cwd: appPath, executable });
  }

  // TODO: Change 'npm ci' command to something using this CLI, like 'npx homey ci'.
  // Needs update: https://github.com/athombv/github-action-homey-app-publish/blob/master/entrypoint.sh#L7
  // Python equavalent: We have to use a third party dependency manager like 'pipenv', 'poetry' or 'pdm'
  // Most tools just pin the version, without checking the file hash (pip freeze, uv lock, pip-compile)
  static async addGitHubWorkflows({ appPath }) {
    throw new Error('Method should not be called for Python: "App.addGitHubWorkflows". (Not Implemented yet)');
  ...
  static async pipInstall({
    appPath,
    packages,
    allowEmpty = false,
    executable = undefined,
  }) {
    const manifest = AppProject.getComposeManifest({ appPath });
    const constrains = manifest.pipFreeze ? manifest.pipFreeze.join('\r\n') : undefined;

    // When no package is defined. Install all packages from `app.json`.
    if (!packages || packages.length === 0) {
      packages = manifest.pipRequirements || [];

      // When this command is ran without defined package(s)
      // and there are no packages in `app.json`, it is probably a user error.
      if (!allowEmpty && packages.length === 0) {
        throw new Error('Manifest does not contain any packages. Start by installing one with `homey app pip install <packageName>`!');
      }
    }

    // Install in virtual environment (uses Docker)
    await PythonCommands.pipInstall(packages, { constrainsTxt: constrains, appPath, executable });

    // Update `app.json` manifest 'pipRequirements'
    await AppPython.modifyPipPackage(packages, AppPython.MODIFY_PIP_PACKAGES_ACTIONS.ADD, { appPath });

    // Update `app.json` manifest 'pipFreeze'
    await AppPython.syncPipFreeze({ appPath, executable });
  }

  static async pipUninstall({ appPath, packages, executable = undefined }) {
    // Update `app.json` manifest 'pipRequirements'
    await AppPython.modifyPipPackage(packages, AppPython.MODIFY_PIP_PACKAGES_ACTIONS.DELETE, { appPath });

    const manifest = AppProject.getComposeManifest({ appPath });
    const pipFreeze = manifest.pipFreeze || [];

    // Uninstall all package
    await PythonCommands.pipUninstall(pipFreeze, { appPath, executable });

    // Reinstall packages based on `app.json`
    // Use `pipRequirements` as requirements.txt
    // Use `pipFreeze` as constrains.txt
    // This will install all 'installed' packages, and keep the versions like before
    ...
    // Update `app.json` manifest 'pipFreeze'
    // if uninstalled package had dependencies, and those are not reinstalled, this pip freeze does not include them.
    await AppPython.syncPipFreeze({ appPath, executable });
  }

  static async pipList({ appPath }) {
    const manifest = AppProject.getComposeManifest({ appPath });
    const pipRequirements = manifest.pipRequirements || [];
    const pipFreeze = manifest.pipFreeze || [];

    if (pipRequirements.length === 0 && pipFreeze.length === 0) {
      Log.error('No packages installed yet. (Try `homey app pip sync` if you think this is incorrect.)');
      return;
    }

    Log.success(`Requirements (${pipRequirements.length === 0 ? 'empty' : pipRequirements.length}):`);
    pipRequirements.forEach(item => {
      Log.info(' -', item);
    });

    Log.success(`Freeze (${pipFreeze.length === 0 ? 'empty' : pipFreeze.length}):`);
    pipFreeze.forEach(item => {
      Log.info(' -', item);
    });
  }

  static MODIFY_PIP_PACKAGES_ACTIONS = {
    ADD: 'add',
    DELETE: 'delete',
  }

  static async modifyPipPackage(requirements, action, { appPath }) {
    ...
    for (const requirement of requirements) {
      const requirementPackageName = requirement.match(/^([A-Z0-9][A-Z0-9._-]*[A-Z0-9]|[A-Z0-9])/i)[0];

      const deleteIndex = pipRequirements.findIndex(req => req.match(/^([A-Z0-9][A-Z0-9._-]*[A-Z0-9]|[A-Z0-9])/i)[0] === requirementPackageName);

      if (deleteIndex !== -1) {
        pipRequirements.splice(deleteIndex, 1);
      }
      if (action === AppPython.MODIFY_PIP_PACKAGES_ACTIONS.ADD) {
        pipRequirements.push(requirement);
      }
    }

    manifest.pipRequirements = pipRequirements;
    await writeFileAsync(path.join(appPath, '.homeycompose', 'app.json'), JSON.stringify(manifest, false, 2));
  }

  static async syncPipFreeze({ appPath, executable = undefined }) {
    // Read mainifest file
    const manifest = AppProject.getComposeManifest({ appPath });
    const originalPipFreeze = manifest.pipFreeze || [];

    // Get current dependency list
    const pipFreezeResults = await PythonCommands.pipFreeze({ appPath, executable });

    if (pipFreezeResults !== originalPipFreeze) {
      // Update the manifest only if necessary
      manifest.pipFreeze = pipFreezeResults;
      await writeFileAsync(path.join(appPath, '.homeycompose', 'app.json'), JSON.stringify(manifest, false, 2));
    }
  }

  static async createRequirementsTxt({ buildPath }) {
    const requirementsPath = path.join(buildPath, 'requirements.txt');

    const manifest = AppProject.getManifest({ appPath: buildPath });
    const installedPackages = manifest.pipRequirements || [];

    await writeFileAsync(requirementsPath, installedPackages.join('\r\n'));
