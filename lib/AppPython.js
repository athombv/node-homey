...
class AppPython extends App {
  ...
  async version(version) {
    if (['patch', 'minor', 'major'].includes(version)) {
      await AppPython.uvCommand(['uv', 'version', '--bump', version], this.path, { onlyForLocal: true }).catch();
    } else {
      await AppPython.uvCommand(['uv', 'version', version], this.path, { onlyForLocal: true }).catch();
    }
  ...
  static async create({ appPath: cwd, globalAnswers }) {
    ...
    await AppPython.uvCommand(['uv', 'init', '--bare', '--no-description', '-q', '--python', '==3.13.*', '.'], appPath, { onlyForLocal: true });
    await AppPython.uvCommand(['uv', 'version', '--active', '-q', '1.0.0'], appPath, { onlyForLocal: true });
  ...
  static async uvCommand(command, projectPath, {
    findLinks, pythonCachePath, dockerSocketPath, onlyForLocal = false,
  } = {}) {
    ...
    const cacheBasePath = pythonCachePath !== undefined ? pythonCachePath : path.join(projectPath, PYTHON_CACHE_FOLDER);
