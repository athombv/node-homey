...
class AppPython extends App {
  ...
  static async executeUvCommand(docker, HOMEY_APP_BUILDER_ID, dockerCommand, createOptions, platform, retry = false) {
    ...
    const result = await runPromise.catch((error) => {
      if (error instanceof Error) {
        if (error.message.includes('was found but does not provide the specified platform')) {
          // The image may be present in overlay2, but not dockerd
          if (retry) {
            throw new StacklessError(`Builder image ${HOMEY_APP_BUILDER_ID} corrupted for platform ${platform}`);
          }
          Log.error(`Builder image not found for platform ${platform}.`);
          Log.error('Pulling image and retrying...');
          retry = true;
        }
        throw error;
    ...
    if (retry) {
      return this.executeUvCommand(docker, HOMEY_APP_BUILDER_ID, createOptions, platform, true);
    }
