...
class AppPython extends App {
  ...
  static async create({ appPath: cwd, globalAnswers }) {
    ...
    await AppPython.uvCommand(['uv', 'init', '--bare', '--no-description', '-q', '.'], appPath, { onlyForLocal: true, skipVersionCheck: true });
    await AppPython.uvCommand(['uv', 'version', '--active', '-q', '1.0.0'], appPath, { onlyForLocal: true, skipVersionCheck: true });
