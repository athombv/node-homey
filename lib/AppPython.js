...
class AppPython extends App {
  ...
  async addDependencies({
    ...
    const oldDependencies = manifest.pythonDependencies ?? [];
    ...
    manifest.pythonDependencies = newDependencies;
    ...
    await this.installDependencies({ findLinks, dockerSocketPath }).catch(async (error) => {
      manifest.pythonDependencies = oldDependencies;
  ...
  async checkPythonVersion(projectPath) {
    ...
    const pythonVersion = manifest.pythonVersion;
    const previousPythonVersion = await fsPromises.readFile(path.join(projectPath, PYTHON_PACKAGES_FOLDER, '.python-version'), 'utf8').catch(() => undefined);
