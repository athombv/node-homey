...
class AthomApi {
  ...
  async _clearProfileCache() {
    const store = await this._store.get();
    delete store.user;
    delete store.profileCache;

    await this._store.set(store);
  }
  ...
  async getProfile({ cache = true } = {}) {
    ...
    const store = await this._store.get();
    const metadata = store.profileCache;
    const hasCachedProfile = Boolean(store.user && metadata?.authKey === this._profileAuthKey);
    const now = Date.now();
    const isFresh = hasCachedProfile && now - metadata.updatedAt < PROFILE_CACHE_TTL;
    const isCoolingDown = hasCachedProfile && now < metadata.retryAfter;

    if ((cache && isFresh) || isCoolingDown) {
      return await this._api.getAuthenticatedUserFromStore();
    }

    // A PAT can belong to a different account than the stored OAuth session.
    if (!hasCachedProfile) {
      await this._clearProfileCache();
    }
    ...
    try {
      profile = await this._api.getAuthenticatedUserFromStore({ $cache: false });
    } catch (err) {
      if (err.statusCode !== 429 || !hasCachedProfile) {
        throw err;
      }

      const currentStore = await this._store.get();
      await this._store.set({
        ...currentStore,
        profileCache: { ...metadata, retryAfter: Date.now() + PROFILE_RATE_LIMIT_COOLDOWN },
      });
    ...
    await this._store.set({
      ...currentStore,
      profileCache: { authKey: this._profileAuthKey, updatedAt: Date.now() },
    });
