...
class AthomApi {
  ...
  _createApi() {
    ...
    this._api = new AthomCloudAPI({
      ...
      onInvalidRefreshToken: this._onInvalidRefreshToken.bind(this),
    });
  }
  ...
  async _initApi() {
    ...
    // Ensure the user is logged in, but keep this command non-interactive and
    // return with an error.
    if (!(await this._api.isLoggedIn())) {
      throw new Error('Please log in with `homey login`.');
    }
  ...
  async _onInvalidRefreshToken() {
    await this._logout();
    throw new Error('Session expired. Please log in again with `homey login`.');
  }
