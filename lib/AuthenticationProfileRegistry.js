...
const CliState = require('../services/CliState');
const LegacyAthomApi = require('../services/AthomApi');
...
class AuthenticationProfileRegistry {
  constructor() {
  ...
  async getClient(name, { allowInteractiveLogin = false, fresh = false } = {}) {
    ...
    if (profileEntry.profile.accountId) {
  ...
  async loginWithOAuth(name, { store = 'settings', replaceAccount = false } = {}) {
    ...
    try {
      ...
      await CliState.completeOAuthAuthenticationProfile(name, source, metadata, {
        replaceAccount,
      });
      this._clients.set(name, client);
  ...
  async migrateAuthenticationProfile(name, to) {
    ...
    if (
      ...
      try {
        ...
        const authenticatedProfile = await client.getProfile();
        const resolvedIdentity = toAccountMetadata(authenticatedProfile);
