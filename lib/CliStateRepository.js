...
class CliStateRepository {
  ...
  async updateContext(name, updater) {
    assertIdentifier(name, 'context name');
  ...
  async replaceContextDirectToken(name, token, store = 'settings') {
    ...
    let previousAuthentication = null;
    ...
    try {
      await Settings.update((settings) => {
        ...
        previousAuthentication = context.homeyAuthentication;
        ...
        state.contextState.contexts[name] = updated;

        if (
          previousAuthentication?.source === 'stored' &&
          previousAuthentication.store === 'settings'
        ) {
          delete state.credentials.entries[previousAuthentication.credentialId];
        }
      });
    ...
    if (
      previousAuthentication?.source === 'stored' &&
      previousAuthentication.store === 'keychain'
    ) {
      await OperatingSystemCredentialStore.remove(previousAuthentication.credentialId);
    }
  ...
  async removeAuthenticationProfile(name) {
    ...
    await Settings.update((settings) => {
      ...
      if (name === DEFAULT_IDENTIFIER && source?.legacy) {
