...
export const DISCOVERY_STRATEGIES = new Set([
  'mdns',
  'cloud',
  'local',
  'localSecure',
...
function applyLegacyViews(namespaces, settings) {
  ...
  const hasStoredContextState = settings.contextState && typeof settings.contextState === 'object';
  const hasLegacyHomey =
...
function isLegacyAuthenticationUsable(homeyApi) {
  return (
    typeof homeyApi?.token?.access_token === 'string' && homeyApi.token.access_token.length > 0
  );
}

async function evaluateAuthenticationProfile(profile, credentials, settings) {
  if (!profile) {
    return {
      usable: false,
...
async function evaluateContextHealth(context, namespaces, settings, requiredCapability = null) {
  const reasons = [];
...
export class CliStateRepository {
  async read() {
    ...
    return {
      ...
      legacy: {
        ...
        hasAuthentication: isLegacyAuthenticationUsable(homeyApi),
      },
    };
  }
  ...
  async listAuthenticationProfiles() {
    ...
    const profilePromises = Object.entries(state.authenticationProfiles.profiles).map(
      async ([name, profile]) => {
        const health = await evaluateAuthenticationProfile(profile, state.credentials, {
          hasLegacyAuthentication: state.legacy.hasAuthentication,
        });
