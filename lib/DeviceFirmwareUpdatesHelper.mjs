...
export default class DeviceFirmwareUpdatesHelper {
  static async selectFirmwareFile({ isZigbeeDriver }) {
    ...
    return path.resolve(process.cwd(), result.firmwarePath);
  }
    ...
    const { hasSleepMode } = await inquirer.prompt([
      {
        type: 'confirm',
        name: 'hasSleepMode',
        message: 'Does this device go in to a sleep mode?',
        default: false,
      },
    ]);
    ...
    const { wakeUpInstruction } = await inquirer.prompt([
      {
        ...
        message: 'How can the user wake the device when it is in sleep mode?',
        validate: (input) => input.length > 0,
      },
    ]);
