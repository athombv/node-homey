...
function assertSelectedAuthenticationIsUsable(
  selection,
  authenticationMode,
  hasExplicitDirectAuthentication,
) {
  if (!selection) {
    return;
  }

  if (authenticationMode === 'homey' && hasExplicitDirectAuthentication) {
    return;
  }

  const capabilityIsUsable = selection.health?.capabilities?.[authenticationMode];

  if (capabilityIsUsable !== false) {
    return;
  }

  const capabilityLabel =
    authenticationMode === 'homey' ? 'direct Homey authentication' : 'account authentication';
  const healthReasons = selection.health?.reasons?.join(' ') ?? '';
  const reasonSuffix = healthReasons ? ` ${healthReasons}` : '';

  throw new Error(`The selected context has no usable ${capabilityLabel}.${reasonSuffix}`);
}

function overlayDirectAuthenticationHealth(health, replacesRoute) {
  if (!health) {
    return null;
  }

  const reasons = health.reasons.filter((reason) => {
    if (reason.startsWith('Direct Homey authentication:')) {
      return false;
    }

    return !(replacesRoute && reason.startsWith('Connection route:'));
  });
  const routeIsUnusable = reasons.some((reason) => {
    return reason.startsWith('Connection route: Homey Cloud targets require');
  });

  return {
    ...health,
    status: routeIsUnusable ? 'unusable' : reasons.length > 0 ? 'degraded' : 'ready',
    capabilities: {
      ...health.capabilities,
      homey: true,
    },
...
async function resolveEffectiveContextFromSelection(argv, selection, scope) {
  ...
  const discardsConfiguredAddress =
    explicitToken && explicitHomeyId && selectedContext?.route?.type === 'address';
  ...
  let route = selectedContext?.route ?? null;
  ...
  } else if (discardsConfiguredAddress) {
    route = null;
  }
  ...
  assertSelectedAuthenticationIsUsable(selection, authMode, explicitToken);
  ...
  const effectiveContext = {
    ...
    route,
    health: explicitToken
      ? overlayDirectAuthenticationHealth(selection?.health ?? null, explicitAddress)
      : (selection?.health ?? null),
  };
