  ...
  async _composeDrivers() {
    ...
    for (let driverIndex = 0; driverIndex < drivers.length; driverIndex++) {
      ...
      if (Array.isArray(driverJson.pair)) {
        ...
        // copy images
        for (let k = 0; k < composePairViews.length; k++) {
          if (  composePairViews[k].indexOf('.png')>-1 ||
                composePairViews[k].indexOf('.jpg')>-1 ||
                composePairViews[k].indexOf('.jpeg')>-1 ) {
            let image = await readFileAsync(path.join(composePairPath, composePairViews[k]));
            await writeFileAsync(path.join(appPairPath, composePairViews[k]), image);
          }
        }
      }
      ...
      if (Array.isArray(driverJson.repair)) {
        ...
        // copy images
        for (let k = 0; k < composeRepairViews.length; k++) {
          if (  composeRepairViews[k].indexOf('.png')>-1 ||
                composeRepairViews[k].indexOf('.jpg')>-1 ||
                composeRepairViews[k].indexOf('.jpeg')>-1 ) {
            let image = await readFileAsync(path.join(composeRepairPath, composeRepairViews[k]));
            await writeFileAsync(path.join(appRepairPath, composeRepairViews[k]), image);
          }
        }
