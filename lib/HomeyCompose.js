...
class HomeyCompose {
  ...
  static async migrateLocales({ appPath }) {
    const compose = new HomeyCompose(appPath);
    await compose.migrateLocales();
  }
    ...
    for (const key of Object.keys(obj)) {
      if (typeof obj[key] === 'string') {
        ...
        try {
          ...
          console.error(`Missing property \`name\` in driver ${driverId}`);
  ...
  */
  ...
  async _composeLocales() {
    ...
    // Merge $drivers, $flow, $capabilities into /app.json
        ...
        for (const [driverId, driver] of Object.entries(appLocale.$drivers)) {
          ...
          if (driver.settings) {
            ...
            for (const [settingId, setting] of Object.entries(driver.settings)) {
              ...
              if (appJsonDriverSetting.type === 'dropdown' && setting.values) {
  ...
  async migrateLocales() {
    const locales = await this._getJsonFiles(path.join(this._appPathCompose, 'locales'));

    // Capabilities
    const capabilitiesPath = path.join(this._appPathCompose, 'capabilities');
    const capabilities = await this._getJsonFiles(capabilitiesPath);
    for (const [capabilityId, capability] of Object.entries(capabilities)) {
      for (const locale of HomeyLib.App.getLocales()) {
        if (capability.title?.[locale]) {
          objectPath.set(locales, `${locale}.$capabilities.${capabilityId}.title`, capability.title[locale]);
        }

        if (capability.units?.[locale]) {
          objectPath.set(locales, `${locale}.$capabilities.${capabilityId}.units`, capability.units[locale]);
        }
      }

      delete capability.title;
      delete capability.units;

      await writeFileAsync(path.join(capabilitiesPath, `${capabilityId}.json`), JSON.stringify(capability, false, 2));
    }

    // Drivers
    const driversPath = path.join(this._appPath, 'drivers');
    const drivers = await this._getChildFolders(driversPath);
    for (const driverId of drivers) {
      // driver.compose.json
      const driverComposePath = path.join(driversPath, driverId, 'driver.compose.json');
      if (await fse.exists(driverComposePath)) {
        const driverComposeJson = await this._getJsonFile(driverComposePath);
        for (const locale of HomeyLib.App.getLocales()) {
          if (driverComposeJson.name?.[locale]) {
            objectPath.set(locales, `${locale}.$drivers.${driverId}.name`, driverComposeJson.name[locale]);
          }
        }

        delete driverComposeJson.name;
        await writeFileAsync(driverComposePath, JSON.stringify(driverComposeJson, false, 2));
      }

      // driver.flow.compose.json
      const driverFlowComposePath = path.join(driversPath, driverId, 'driver.flow.compose.json');
      if (await fse.exists(driverFlowComposePath)) {
        const driverFlowComposeJson = await this._getJsonFile(driverFlowComposePath);

        for (const flowType of ['triggers', 'conditions', 'actions']) {
          const flowCards = driverFlowComposeJson[flowType];
          if (!Array.isArray(flowCards)) continue;

          for (const flowCard of flowCards) {
            // title
            for (const locale of HomeyLib.App.getLocales()) {
              if (flowCard.title?.[locale]) {
                objectPath.set(locales, `${locale}.$flow.${flowType}.${flowCard.id}.title`, flowCard.title[locale]);
              }
            }
            delete flowCard.title;

            // titleFormatted
            for (const locale of HomeyLib.App.getLocales()) {
              if (flowCard.titleFormatted?.[locale]) {
                objectPath.set(locales, `${locale}.$flow.${flowType}.${flowCard.id}.titleFormatted`, flowCard.titleFormatted[locale]);
              }
            }
            delete flowCard.titleFormatted;

            // hint
            for (const locale of HomeyLib.App.getLocales()) {
              if (flowCard.hint?.[locale]) {
                objectPath.set(locales, `${locale}.$flow.${flowType}.${flowCard.id}.hint`, flowCard.hint[locale]);
              }
            }
            delete flowCard.hint;

            // args
            if (Array.isArray(flowCard.args)) {
              for (const arg of flowCard.args) {
                for (const locale of HomeyLib.App.getLocales()) {
                  if (arg.title?.[locale]) {
                    objectPath.set(locales, [locale, '$flow', flowType, flowCard.id, 'args', arg.name, 'title'], arg.title[locale]);
                  }

                  if (arg.placeholder?.[locale]) {
                    objectPath.set(locales, [locale, '$flow', flowType, flowCard.id, 'args', arg.name, 'placeholder'], arg.placeholder[locale]);
                  }
                }

                delete arg.title;
                delete arg.placeholder;
              }
            }

            // tokens
            if (Array.isArray(flowCard.tokens)) {
              for (const token of flowCard.tokens) {
                for (const locale of HomeyLib.App.getLocales()) {
                  if (token.title?.[locale]) {
                    objectPath.set(locales, [locale, '$flow', flowType, flowCard.id, 'tokens', token.name, 'title'], token.title[locale]);
                  }

                  if (token.example?.[locale]) {
                    objectPath.set(locales, [locale, '$flow', flowType, flowCard.id, 'tokens', token.name, 'example'], token.example[locale]);
                  }
                }
        ...
        await writeFileAsync(driverFlowComposePath, JSON.stringify(driverFlowComposeJson, false, 2));

        // driver.settings.compose.json
        const driverSettingsComposePath = path.join(driversPath, driverId, 'driver.settings.compose.json');
        if (await fse.exists(driverSettingsComposePath)) {
          const driverSettingsComposeJson = await this._getJsonFile(driverSettingsComposePath);

          // Flatten settings
          const driverSettingsComposeJsonFlat = driverSettingsComposeJson.reduce((acc, setting, i) => {
            if (!setting.id) {
              Log.warning(`Missing Setting ID in driver ${driverId}.settings.${i} (${setting.type})`);
              return acc;
            }
          ...
          for (const [settingId, setting] of Object.entries(driverSettingsComposeJsonFlat)) {
            if (!settingId) continue;

            for (const locale of HomeyLib.App.getLocales()) {
              if (setting.label?.[locale]) {
                objectPath.set(locales, [locale, '$drivers', driverId, 'settings', settingId, 'label'], setting.label[locale]);
              }

              if (setting.hint?.[locale]) {
                objectPath.set(locales, [locale, '$drivers', driverId, 'settings', settingId, 'hint'], setting.hint[locale]);
              }

              if (setting.values) {
                for (const [valueId, value] of Object.entries(setting.values)) {
                  if (value.label?.[locale]) {
                    objectPath.set(locales, [locale, '$drivers', driverId, 'settings', settingId, 'values', valueId, 'label'], value.label[locale]);
                  }
                }
              }
            }
            ...
            await writeFileAsync(driverSettingsComposePath, JSON.stringify(driverSettingsComposeJson, false, 2));
          }
        }
      ...
      // Flow
      const flowPath = path.join(this._appPathCompose, 'flow');
      // TODO
    }

    // Write new Locales
    for (const [localeId, locale] of Object.entries(locales)) {
      await fse.ensureDir(path.join(this._appPathCompose, 'locales'));
      await writeFileAsync(path.join(this._appPathCompose, 'locales', `${localeId}.json`), JSON.stringify(locale, false, 2));
    }
  }
