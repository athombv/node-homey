  ...
  */
  async _composeWidgets() {
    ...
    for (let widgetIndex = 0; widgetIndex < widgets.length; widgetIndex++) {
      ...
      // merge json
      let widgetJson = await this._getJsonFile(
      ...
      // merge settings
      try {
        widgetJson.settings = await this._getJsonFile(
          path.join(this._appPath, 'drivers', widgetId, 'widgets.settings.compose.json'),
        );
      } catch (err) {
        if (err.code !== 'ENOENT') throw new Error(err);
      }
      ...
      Log.info(`Added Driver \`${widgetId}\``);
    }
  }
