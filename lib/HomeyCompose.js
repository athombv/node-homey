...
class HomeyCompose {
    ...
    for (let i = 0; i < drivers.length; i++) {
      ...
      function extendSetting(settingsTemplates, settingObj) {
        if (settingObj.type === 'group') {
          for (const childSettingId in settingObj.children) {
      ...
      try {
        ...
        if (Array.isArray(driverJson.settings)) {
          for (const settingId in driverJson.settings) {
      ...
      if (Array.isArray(driverJson.pair)) {
        ...
        for (let j = 0; j < driverJson.pair.length; j++) {
          ...
          if (driverJson.$pairOptions) {
            for (const viewId in driverJson.$pairOptions) {
              const options = driverJson.$pairOptions[viewId];

              const view = _.findWhere(driverJson.pair, { id: viewId });
              if (view) {
  ...
  */
  ...
  async _composeScreensavers() {
    ...
    for (const screensaverId in screensavers) {
