...
class ZWave {
  ...
  static async autocompleteDriver({ driverPath, driverJson }) {
    ...
    // download image
    const imageUrl = sigmaJson.Image || `https://products.z-wavealliance.org/ProductImages/Index?productName=${sigmaJson.CertificationNumber}`;
    if (imageUrl) {
      const agent = new https.Agent({
        rejectUnauthorized: false,
      });

      const res = await fetch(imageUrl, { agent });
      if (res.ok) {
    ...
    // set properties
    zwJson.manufacturerId = parseInt(sigmaJson.ManufacturerId, 10);
    zwJson.productTypeId = [parseInt(sigmaJson.ProductTypeId, 10)];
    zwJson.productId = [parseInt(sigmaJson.ProductId, 10)];
