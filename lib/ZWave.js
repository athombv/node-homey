...
class ZWave {
  static logAutocompleteDriverHint() {
    Log.success(
  ...
  static parseRequiredIntegerInput(value, label) {
  ...
  static parseRequiredIntegerListInput(value, label) {
    const normalizedValue = String(value ?? '').trim();
  ...
  static buildDriverAutocompleteResult({
    exclusionDescription,
    inclusionDescription,
    manufacturerId,
    productId,
    productTypeId,
    sigmaAllianceProductDocumentation,
    sigmaAllianceProductId,
  } = {}) {
    const zwaveJson = {
      manufacturerId: ZWave.parseRequiredIntegerInput(manufacturerId, 'Manufacturer ID'),
      productId: ZWave.parseRequiredIntegerListInput(productId, 'Product ID'),
      productTypeId: ZWave.parseRequiredIntegerListInput(productTypeId, 'Product Type ID'),
    };
    const settingsJson = [];
  ...
  static async autocompleteDriver() {
    ...
    */
    ...
    return ZWave.buildDriverAutocompleteResult({
      exclusionDescription,
      inclusionDescription,
      manufacturerId,
      productId,
        ...
        (response) => {
          ...
          response.on('end', () => {
            if (!response.statusCode || response.statusCode < 200 || response.statusCode >= 300) {
              return settleReject(new Error(response.statusMessage || 'Request failed'));
            }

            try {
              return settleResolve(JSON.parse(data));
            } catch (error) {
