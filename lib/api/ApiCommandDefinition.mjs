...
const HOMEY_MANAGER_WHITELIST = new Set(['devices']);
...
export function getHomeyManagerDefinition(managerIdCamelCase) {
  ...
  const managerEntry = Object.entries(specification.managers || {})
    .find(([, manager]) => manager.idCamelCase === managerIdCamelCase);
  ...
  const defaultOperation = operations.find((operation) => operation.id === 'getDevices')
    || operations[0]
    || null;
