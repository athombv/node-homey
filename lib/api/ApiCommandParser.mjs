...
export function applyOperationOptions(yargs, operation) {
  let hasRootBodyParameter = false;
  let rootBodyParameterRequired = false;

  for (const [parameterName, parameter] of Object.entries(operation.parameters || {})) {
    if (parameter.in === 'body' && parameter.root === true) {
      hasRootBodyParameter = true;
      rootBodyParameterRequired = rootBodyParameterRequired || parameter.required === true;
      continue;
    }
    ...
    yargs.option(optionName, {
      ...
      description: `${parameter.in} parameter${parameter.required === true ? ' (required)' : ''}`,
    });
  }
