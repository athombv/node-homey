...
import { HomeyUsb } from '../HomeyUsb.js';
...
function validateAuthFlags({ token, address, homeyId, usb }) {
  if (usb && address) {
    throw new Error(
      'Cannot combine USB mode with --address. Use --no-usb for an explicit address.',
    );
  }
...
export async function resolveRequestedHomey(homeyId, { usb = false } = {}) {
  if (typeof homeyId === 'string' && homeyId.length > 0) {
    return await AthomApi.getHomey(homeyId, { usb });
  }
  ...
  return await AthomApi.getHomey(activeHomey.id, { usb });
}
...
async function authenticateHomey(homey, { usb = false } = {}) {
  try {
    if (usb) {
      const api = await AthomApi._initApi();

      return await HomeyUsb.authenticate(homey, { api });
    }
...
async function createTokenHomeyApiForHomey({ token, homeyId, usb }) {
  const homey = await AthomApi.getHomey(homeyId, { usb });

  if (usb) {
    return HomeyUsb.createClient(homey, { token });
  }
...
export async function createHomeyApiClient({ token, address, homeyId, usb = false }) {
  validateAuthFlags({ token, address, homeyId, usb });
  ...
  if (token && homeyId) {
    return await createTokenHomeyApiForHomey({ token, homeyId, usb });
  }

  const homey = await resolveRequestedHomey(homeyId, { usb });
  return await authenticateHomey(homey, { usb });
}

export async function diagnoseHomeyStrategies({ homeyId, usb = false } = {}) {
  let homey = await resolveRequestedHomey(homeyId);
  const preferredStrategyIds = usb ? ['usb'] : getPreferredAuthenticateStrategy(homey);
  const attemptedStrategyIds = usb ? ['usb'] : getDiagnoseStrategyOrder(homey);
  const results = [];
  ...
  for (const strategyId of attemptedStrategyIds) {
    ...
    try {
      if (usb) {
        homey = await AthomApi.getHomey(homey.id, { usb: true });
        api = await authenticateHomey(homey, { usb: true });
        await api.sessions.getSessionMe({ $socket: false });
      } else {
