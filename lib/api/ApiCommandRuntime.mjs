...
import { resolveEffectiveContext } from '../EffectiveContextResolver.mjs';
...
async function createTokenHomeyApiForHomey({ token, homeyId, accountClient = AthomApi }) {
...
export async function createHomeyApiClient(options = {}) {
  const { token, address, homeyId, context, auth } = options;
  const usesContextResolution =
    context !== undefined || auth !== undefined || Boolean(process.env.HOMEY_CONTEXT);

  if (usesContextResolution) {
    return await createContextHomeyApiClient(options);
  }
...
async function createContextHomeyApiClient(argv) {
  const { effectiveContext, accountClient } = await resolveEffectiveContext(argv, {
    scope: 'homey',
  });
...
export async function diagnoseHomeyStrategies({ homeyId, context, auth } = {}) {
  ...
  if (context !== undefined || auth !== undefined || process.env.HOMEY_CONTEXT) {
    const resolution = await resolveEffectiveContext(
      {
        homeyId,
        context,
        auth: auth === 'homey' ? 'homey' : 'account',
      },
      { scope: 'homey' },
    );
