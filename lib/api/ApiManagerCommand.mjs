...
function resolveDefaultOperation(managerDefinition, preferredDefaultOperationIds = []) {
  for (const operationId of preferredDefaultOperationIds) {
    const operation = managerDefinition.operations.find(
  ...
  return (
    managerDefinition.operations.find(
      (operation) => operation.id === managerDefinition.defaultOperationId,
    ) || null
  );
}
    ...
    builder: (yargs) => {
      ...
      managerDefinition.operations.forEach((operation) => {
        yargs.command(
          ...
          (operationYargs) => applyOperationOptions(operationYargs, operation).help(),
          async (argv) => {
            try {
