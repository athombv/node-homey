...
class Translate {
  ...
  async translateWithOpenAI({
    ...
    const App = this._app.constructor;
...
You can create an API Key on https://platform.openai.com/api-keys.`);
    ...
    if (file) {
      ...
      if (App.hasHomeyCompose({ appPath: this._app.path })) {
        jsonFiles.add(path.join(this._app.path, '.homeycompose', 'app.json'));
      } else {
        jsonFiles.add(path.join(this._app.path, 'app.json'));
      }

      await this._walkSync(this._app.path, jsonFiles);
    }

    Log(`Found ${jsonFiles.size} JSON files to translate...`);

    // Get App name
    const manifest = App.getManifest({ appPath: this._app.path });
    ...
    const enLocalePath = path.join(this._app.path, 'locales', 'en.json');
    ...
    if (enLocalePathExists) {
      ...
      await Promise.all(Object.values(languages).map(async language => {
        return this._pqueue.add(async () => {
          const translatedLocalePath = path.join(this._app.path, 'locales', `${language}.json`);
    ...
    const readme = await fs.promises.readFile(path.join(this._app.path, 'README.txt'), 'utf8');
    await Promise.all(Object.values(languages).map(async language => {
      return this._pqueue.add(async () => {
        const translatedReadmePath = path.join(this._app.path, `README.${language}.txt`);
  ...
  _applyTranslationOrder(obj, path, currentKeys = []) {
    ...
    const sortedKeys = [...allKeys].sort((a, b) => {
      ...
      if (isExistingA || isExistingB) {
        ...
        const indexExisting = Translate.LANGUAGES.indexOf(existingKey);
        const indexNew = Translate.LANGUAGES.indexOf(newKey);

        if (indexExisting !== -1 && indexNew !== -1) {
          // Find the correct position for the new key relative to existing keys
          // that are also in Translate.LANGUAGES.
        ...
        // If one is not in Translate.LANGUAGES, put the new one last
