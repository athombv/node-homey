...
${readme}`,
   ...
   * Each translation object has an 'en' key containing a string or an array of strings.
  ...
  _extractTranslationTasks(obj, path = [], languages) {
    ...
    if (typeof obj === 'object' && obj !== null) {
      ...
      const isTranslationObject =
        keys.includes('en') && (typeof obj.en === 'string' || isStringArray);
      ...
      for (const key of keys) {
        ...
        tasks.push(...this._extractTranslationTasks(obj[key], newPath, languages));
      }
    }
  ...
  _applyTranslationOrder(obj, path, currentKeys = []) {
    ...
    for (let i = 0; i < path.length - 1; i++) {
      ...
      if (current[key] === undefined || current[key] === null || typeof current[key] !== 'object') {
        throw new Error(
          `Invalid path: "${path.slice(0, i + 1).join('.')}" does not exist or is not an object`,
        );
      }
    ...
    for (const task of tasks) {
      ...
      for (const lang of task.missing) {
        ...
        if (Array.isArray(task.source)) {
          ...
          for (const text of task.source) {
            translated.push(await this._translateOne(lang, text, appName));
          }
        } else {
          translated = await this._translateOne(lang, task.source, appName);
        }
