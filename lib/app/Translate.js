...
You can create an API Key on https://platform.openai.com/api-keys.`);
    ...
    if (file) {
      ...
      // Note: App.hasHomeyCompose is used in App.js. We need to handle it or pass the info.
      // Since we don't want circular dependency, let's just check for the folder here or pass it from App.js.
      // For now, let's use a simple check.
      const hasHomeyCompose = fs.existsSync(path.join(this.path, '.homeycompose'));
    ...
    if (fs.existsSync(composeAppJsonPath)) {
      manifest = JSON.parse(await fs.promises.readFile(composeAppJsonPath, 'utf8'));
    } else {
      manifest = JSON.parse(await fs.promises.readFile(appJsonPath, 'utf8'));
    }
    ...
    const readmePath = path.join(this.path, 'README.txt');
    const readmeExists = !!await fs.promises.stat(readmePath).catch(() => null);
    if (readmeExists) {
...
${readme}`,
   ...
   */
  _applyTranslationOrder(obj, path) {
  ...
  async _translateFile({
    ...
    for (const task of tasks) {
      for (const lang of task.missing) {
        const translated = await this._translateOne({
          lang,
          text: task.source,
          appName,
          openai,
      ...
      this._applyTranslationOrder(json, task.path);
    }
...
Translate.LANGUAGES_TRANSLATE = LANGUAGES_TRANSLATE;
